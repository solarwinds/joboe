...
public class CustomMetricsCollectorTest {
  ...
  public void testIncrementMetrics() throws Exception {
    ...
    CustomMetricsCollector.INSTANCE.recordIncrementMetrics(
        "test-1", 1, Collections.singletonMap("key-1", "1"));
    CustomMetricsCollector.INSTANCE.recordIncrementMetrics(
        "test-1", 2, Collections.singletonMap("key-1", "1"));
    CustomMetricsCollector.INSTANCE.recordIncrementMetrics(
        "test-1", 3, Collections.singletonMap("key-1", "1"));

    CustomMetricsCollector.INSTANCE.recordIncrementMetrics(
        "test-1", 2, Collections.singletonMap("key-1", "2"));
    ...
    List<? extends MetricsEntry<?>> collectMetricsEntries =
        CustomMetricsCollector.INSTANCE.collectMetricsEntries();
    ...
    assertEquals(
        6,
        (long)
            entriesByMetricsKey.get(
                new MetricKey("test-1", Collections.singletonMap("key-1", "1"))));
    assertEquals(
        2,
        (long)
            entriesByMetricsKey.get(
                new MetricKey("test-1", Collections.singletonMap("key-1", "2"))));
  ...
  public void testSummaryMetrics() throws Exception {
    ...
    CustomMetricsCollector.INSTANCE.recordSummaryMetric(
        "test-1", 1, 1, Collections.singletonMap("key-1", "1"));
    CustomMetricsCollector.INSTANCE.recordSummaryMetric(
        "test-1", 1, 2, Collections.singletonMap("key-1", "1"));
    CustomMetricsCollector.INSTANCE.recordSummaryMetric(
        "test-1", 1, 3, Collections.singletonMap("key-1", "1"));

    CustomMetricsCollector.INSTANCE.recordSummaryMetric(
        "test-1", 1, 2, Collections.singletonMap("key-1", "2"));
    ...
    List<? extends MetricsEntry<?>> collectMetricsEntries =
        CustomMetricsCollector.INSTANCE.collectMetricsEntries();
    assertEquals(5, collectMetricsEntries.size());
    Map<MetricKey, SummaryDoubleMeasurement> entriesByMetricsKey =
        new HashMap<MetricKey, SummaryDoubleMeasurement>();
    ...
    assertEquals(
        3.0,
        entriesByMetricsKey
            .get(new MetricKey("test-1", Collections.singletonMap("key-1", "1")))
            .getSum());
    assertEquals(
        6,
        entriesByMetricsKey
    ...
    assertEquals(
        1.0,
        entriesByMetricsKey
            .get(new MetricKey("test-1", Collections.singletonMap("key-1", "2")))
            .getSum());
    assertEquals(
        2,
        entriesByMetricsKey
  ...
  public void testMetricLimit() {
    ...
    CustomMetricsCollector.INSTANCE.recordIncrementMetrics(
        "test0", 1, null); // OK as this name already exists
    CustomMetricsCollector.INSTANCE.recordIncrementMetrics(
        "test" + CustomMetricsCollector.limit,
        1,
        null); // NOT OK as this name is new and limit has reached
    ...
    assertEquals(
        ...
        (long)
            entriesByMetricsKey.get(
                new MetricKey(
                    "test0", null))); // once from loop and another one in the manual increment
    assertFalse(
        entriesByMetricsKey.containsKey(
            new MetricKey(
                "test" + CustomMetricsCollector.limit,
                null))); // not exist as the entry should be dropped due to limit
    ...
    testSettingsReader.put(
        new SettingsMockupBuilder()
            ...
            .withSettingsArg(SettingsArg.MAX_CUSTOM_METRICS, 1)
            .build());
    CustomMetricsCollector.INSTANCE.recordIncrementMetrics("test-1", 1, null); // ok
    CustomMetricsCollector.INSTANCE.recordIncrementMetrics(
        "test-2", 1, null); // not ok, limit exceeded
    ...
    testSettingsReader.put(
        new SettingsMockupBuilder()
            ...
            .build());
    CustomMetricsCollector.INSTANCE.recordIncrementMetrics(
        "test-3", 1, null); // ok now as limit has increased (reverted to default)
  ...
  public void testTagLimit() {
    ...
    CustomMetricsCollector.INSTANCE.recordIncrementMetrics(
        "test", 1, tags); // OK as the tags limit is not exceeded yet
    List<? extends MetricsEntry<?>> collectMetricsEntries =
        CustomMetricsCollector.INSTANCE.collectMetricsEntries();
    ...
    tags.put("extra", "tag");
    CustomMetricsCollector.INSTANCE.recordIncrementMetrics(
        "test", 1, tags); // NOT OK as the tags limit exceeded
    collectMetricsEntries = CustomMetricsCollector.INSTANCE.collectMetricsEntries();
