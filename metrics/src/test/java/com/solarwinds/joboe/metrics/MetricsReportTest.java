...
public class MetricsReportTest {
  ...
  public void testReport() throws Exception {
    ...
    Map<MetricsCategory, List<? extends MetricsEntry<?>>> collectedEntries =
        new HashMap<MetricsCategory, List<? extends MetricsEntry<?>>>();

    List<MeasurementMetricsEntry<?>> measurementEntries =
        new ArrayList<MeasurementMetricsEntry<?>>();
    ...
    histogramEntry =
        new HistogramMetricsEntry(
            "a",
            Collections.singletonMap("layer", "a"),
            new MockHistogramAdapter(10.0, 20, 30, 40, 50, 60.0, 70, 80));
    testSpanMetrics.add(histogramEntry);
    histogramEntries.add(histogramEntry);

    histogramEntry =
        new HistogramMetricsEntry(
            "b", null, new MockHistogramAdapter(100.0, 200, 300, 400, 500, 600.0, 700, 800));
    testSpanMetrics.add(histogramEntry);
    histogramEntries.add(histogramEntry);

    testSpanMetrics.add(
        new TopLevelMetricsEntry<Boolean>(
            SpanMetricsCollector.TRANSACTION_NAME_OVERFLOW_LABEL, true));
    ...
    measurementEntry =
        new SimpleMeasurementMetricsEntry("b", Collections.singletonMap("layer", "b"), 100.123);
    testLayerCountMetrics.add(measurementEntry);
    measurementEntries.add(measurementEntry);

    histogramEntry =
        new HistogramMetricsEntry("c", null, new MockHistogramAdapter(1.0, 2, 3, 4, 5, 6.0, 7, 8));
    ...
    SummaryMeasurementMetricsEntry summaryMeasurementMetricsEntry =
        new SummaryMeasurementMetricsEntry(
            new MetricKey("custom-1", Collections.singletonMap("tag-1", "A")), customMeasurement);
    ...
    assertEquals(
        true, postedBuiltinMetrics.get(SpanMetricsCollector.TRANSACTION_NAME_OVERFLOW_LABEL));
    assertMetricEntries(
        measurementEntries, (List<Map<String, ?>>) postedBuiltinMetrics.get("measurements"));
    assertMetricEntries(
        histogramEntries, (List<Map<String, ?>>) postedBuiltinMetrics.get("histograms"));
    ...
    assertMetricEntries(
        testCustomMetrics, (List<Map<String, ?>>) postedCustomMetrics.get("measurements"));
    assertNull(postedCustomMetrics.get("histograms"));
  }

  /**
   * when there are too many metric entry (exceeding MAX_MEASUREMENT_ENTRY_COUNT), it will then drop
   * some entries. It will drop according to the natural ordering as defined in <code>
   * MetricsCategory</code> (the last category will get dropped first etc), which in this case the
   * jmx entries will be dropped first
   *
   * @throws Exception
   */
  @Test
  public void testReportTooManyJmx() throws Exception {
    ...
    List<SimpleMeasurementMetricsEntry> testJmxEntries =
        new ArrayList<SimpleMeasurementMetricsEntry>();
    ...
    List<SimpleMeasurementMetricsEntry> testLayerEntries =
        new ArrayList<SimpleMeasurementMetricsEntry>();
    ...
    List<Map<String, ?>> postedMeasurements =
        (List<Map<String, ?>>) postedMetrics.get("measurements");

    assertMetricEntries(
        testLayerEntries,
        postedMeasurements.subList(
            0, layerMetricCount)); // the layer entries should all make it as it has higher priority
    // than JMX
    int expectedPostedJmxEntriesCount =
        MetricsReporter.MAX_MEASUREMENT_ENTRY_COUNT
            - layerMetricCount; // only a sublist of jmx entries can make it, the remaining counts
    // left after the layer entries
    assertMetricEntries(
        testJmxEntries.subList(0, expectedPostedJmxEntriesCount),
        postedMeasurements.subList(
            layerMetricCount,
            postedMeasurements.size())); // only a sublist of jmx entries can make it
  }
  ...
  public void testLongTags() throws Exception {
    ...
    List<SimpleMeasurementMetricsEntry> measurementEntries =
        new ArrayList<SimpleMeasurementMetricsEntry>();
    List<HistogramMetricsEntry> histogramEntries = new ArrayList<HistogramMetricsEntry>();

    final List<HistogramMetricsEntry> testLayerHistogramMetrics =
        new ArrayList<HistogramMetricsEntry>();
    HistogramMetricsEntry histogramEntry;

    histogramEntry =
        new HistogramMetricsEntry(
            "a",
            Collections.singletonMap(longName, longValue),
            new MockHistogramAdapter(10.0, 20, 30, 40, 50, 60.0, 70, 80));
    ...
    measurementEntry =
        new SimpleMeasurementMetricsEntry(
            longName, Collections.singletonMap(longName, longValue), 100.123);
    testLayerCountMetrics.add(measurementEntry);
    measurementEntries.add(measurementEntry);
    ...
    assertMetricEntries(
        measurementEntries, (List<Map<String, ?>>) postedMetrics.get("measurements"));
  ...
  private void assertMetricEntries(
      List<? extends MetricsEntry<?>> expectedEntries, List<Map<String, ?>> list) {
        ...
        for (String expectedTagKey : expectedMeasurement.getTags().keySet()) {
          ...
          if (expectedTagValue instanceof String) {
            if (((String) expectedTagValue).length() > MetricsReporter.MAX_TAG_VALUE_LENGTH) {
              expectedTagValue =
                  ((String) expectedTagValue).substring(0, MetricsReporter.MAX_TAG_VALUE_LENGTH);
            }
          }
  ...
  private void assertBasicKeys(Map<String, Object> postedMetrics) {
    ...
    assertEquals(
        INTERVAL / 1000, postedMetrics.get("MetricsFlushInterval")); // from millisec to sec
  }
}
