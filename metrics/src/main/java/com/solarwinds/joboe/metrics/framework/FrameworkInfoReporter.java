...
import com.solarwinds.joboe.metrics.SystemReporter;
...
class FrameworkInfoReporter extends SystemReporter<String, Object> {
  ...
  private final ClientLoggingCallback<Result> loggingCallback =
      new ClientLoggingCallback<Result>("framework init");
    ...
    if (!frameworkKvs.isEmpty()) {
      ...
      logger.debug(
          getClass().getSimpleName()
              + " reported jar with manifest info of size ["
              + frameworkKvs.size()
              + "]");
    }
  }
  ...
  public void reportData(Map<String, Object> collectedData, long interval) {
    frameworkKvs.putAll(
        collectedData); // buffer up the framework to send them in one call for better performance
  }
}
