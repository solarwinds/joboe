 ...
 *
 * <p>Based on different mBean and attribute, this locator will return the corresponding <code>
 * AttributeProcessor</code>
 *
 * @author Patson Luk
 */
public class AttributeProcessorLocator {
   ...
   */
  public static AttributeProcessor getProcessor(
      ObjectName mBeanName, String attributeName, MBeanServer mbs) {
    if (OS_OBJECT_NAME.equals(mBeanName)
        && attributeName.equals("ProcessCpuLoad")) { // special processor for
      // java.lang:type=OperatingSystem[ProcessCpuLoad]
      return CpuLoadAttributeProcessor.getInstance(mbs);
    } else if (OS_OBJECT_NAME.equals(mBeanName)
        && attributeName.equals(
            "ProcessCpuTime")) { // IBM 1.5 does not have this attribute, so this attribute is
      // optional (should not trigger error message if missing)
