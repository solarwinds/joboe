...
/**
 * A CPU load attribute processor that computes the CPU load if such an attribute is not supported
 * by the system. CPU load is only support in Java 1.7+
 *
 * <p>Take note that this should be a singleton as it is stateful to a specific JVM instance
 * (process).
 *
 * @author Patson Luk
 */
class CpuLoadAttributeProcessor extends AttributeProcessor {
  ...
  @Override
  /**
   * Attempt to retrieve the CPU load information from MBeanServer. If not available, try to compute
   * the value.
   */
  public Map<String, Object> process(ObjectName mBeanName, String attributeName)
    ...
    try {
      ...
      // System.out.println("Compared with computation result: " + computeCpuLoad()); //just for
      // comparison!

    } catch (AttributeNotFoundException e) {
      // try to compute the CPU load if the attribute is not available in the mBean as it is only
  ...
  /**
   * Compute the Cpu load using logic in JConsole source. Referenced from {@link <a
   * href="http://knight76.blogspot.ca/2009/05/how-to-get-java-cpu-usage-jvm-instance.html">...</a>}
   *
  ...
  private synchronized Double computeCpuLoad() {
    ...
    try {
      ...
      if (processCpuTimeObj != null) {
        ...
        //                System.out.println("Process: " + elapsedCpu + " Uptime: " + elapsedUpTime
        // + " Processor count: " + availableProcessorCount);

        if (elapsedUpTime > 0L) {
          BigDecimal cpuUsage =
              new BigDecimal(elapsedCpu / (elapsedUpTime * 1000000.0 * availableProcessorCount));
