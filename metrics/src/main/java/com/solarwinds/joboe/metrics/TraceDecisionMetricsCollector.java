...
/**
 * Sub metrics collector that collects metrics recorded by {@link TraceDecisionUtil} such as
...
public class TraceDecisionMetricsCollector extends AbstractMetricsEntryCollector {
  ...
  public List<? extends MetricsEntry<?>> collectMetricsEntries() {
    List<SimpleMeasurementMetricsEntry> layerMetricsEntries =
        new ArrayList<SimpleMeasurementMetricsEntry>();

    layerMetricsEntries.add(
        getMetricEntry(TraceDecisionUtil.MetricType.THROUGHPUT, "RequestCount"));
    layerMetricsEntries.add(
        getMetricEntry(
            TraceDecisionUtil.MetricType.TOKEN_BUCKET_EXHAUSTION, "TokenBucketExhaustionCount"));
    layerMetricsEntries.add(getMetricEntry(TraceDecisionUtil.MetricType.TRACE_COUNT, "TraceCount"));
    layerMetricsEntries.add(
        getMetricEntry(TraceDecisionUtil.MetricType.SAMPLE_COUNT, "SampleCount"));
    layerMetricsEntries.add(
        getMetricEntry(TraceDecisionUtil.MetricType.THROUGH_TRACE_COUNT, "ThroughTraceCount"));
    layerMetricsEntries.add(
        getMetricEntry(TraceDecisionUtil.MetricType.TRIGGERED_TRACE_COUNT, "TriggeredTraceCount"));

    Map<String, TraceConfig> layerConfigs = TraceDecisionUtil.consumeLastTraceConfigs();

    Map<Entry<String, Object>, Integer> layerSampleRate =
        new HashMap<Entry<String, Object>, Integer>();
    Map<Entry<String, Object>, Integer> layerSampleSource =
        new HashMap<Entry<String, Object>, Integer>();
  ...
  private SimpleMeasurementMetricsEntry getMetricEntry(
      TraceDecisionUtil.MetricType metricType, String keyName) {
    return new SimpleMeasurementMetricsEntry(
        keyName, Collections.emptyMap(), TraceDecisionUtil.consumeMetricsData(metricType));
  }

  private List<SimpleMeasurementMetricsEntry> convertToMetricsEntries(
      Map<Entry<String, Object>, Integer> data, String keyName) {
