...
import com.solarwinds.joboe.metrics.jmx.processor.AttributeProcessor;
import com.solarwinds.joboe.metrics.jmx.processor.AttributeProcessorLocator;
...
/**
 * Sub metrics collector that collects metrics from JMX's mBeans. Take note that each {@link
 * #collect()} call would return information collected from each of the {@link JMXScope} passed in
 * via the constructor as {@code ConfigProperty.MONITOR_JMX_SCOPES} in {@link ConfigContainer}
 *
 * @author Patson Luk
 */
class JMXCollector extends AbstractMetricsEntryCollector {
  ...
  public JMXCollector(ConfigContainer configs) throws InvalidConfigException {
    ...
    if (scopesString == null) {
      throw new InvalidConfigException(
          "No JMX scope was defined! No JMX metrics would be collected");
    } else {
    ...
    if (configs.containsProperty(ConfigProperty.MONITOR_JMX_MAX_ENTRY)) {
      ...
      if (maxEntryCount < 0) {
        throw new InvalidConfigException(
            ConfigProperty.MONITOR_JMX_MAX_ENTRY
                + " should not be negative but found: "
                + maxEntryCount);
      }
   ...
   */
  private static List<JMXScope> parseScopesString(String scopesString)
      throws InvalidConfigException {
    ...
    try {
      ...
      for (Object keyAsObject : object.keySet()) {
        ...
        if (attributeObj instanceof String) {
          ...
          logger.warn(
              "Unexpected jmx scope value of type [" + attributeObj.getClass().getName() + "]");
   ...
   *
   * @return a Map with key as the attribute name/key and value as the attribute values from the JMX
   *     mBeans
   */
  public Map<MetricKey, Number> collect()
    ...
    try {
      if (systemClassLoader != null && existingContextLoader == null) {
        Thread.currentThread()
            .setContextClassLoader(
                systemClassLoader); // set system class loader to context, it should have better
        // access to classes than the null (bootstrap) loader
      }
      return collect(ManagementFactory.getPlatformMBeanServer());
    } catch (JMRuntimeException e) {
      // known issue for some app server, which the JMX class might not be available yet during
      // startup see https://github.com/librato/joboe/issues/675
      if (e.getCause() instanceof ClassNotFoundException) {
        logger.debug(
            "Cannot load JMX impl class: "
                + e.getMessage()
                + " the class might be available later on",
            e);
      ...
      Thread.currentThread()
          .setContextClassLoader(existingContextLoader); // reset back to whatever it was
    }
  }
   ...
   * Collects information from the JMX mBeans
   *
   * @return a Map with key as the attribute name/key and value as the numeric attribute values from
   *     the JMX mBeans
   */
  Map<MetricKey, Number> collect(MBeanServer mbs)
      throws IntrospectionException,
          InstanceNotFoundException,
          ReflectionException,
          AttributeNotFoundException,
    ...
    for (JMXScope scope : scopes) {
      ...
      try {
        ...
        Set<ObjectName> names =
            mbs.queryNames(
                new ObjectName(scope.getObjectName()),
                null); // query a list of mBean names based on the input <code>ObjectName<code>
        ...
        for (ObjectName name : names) { // iterate through each mBean
          ...
          if (scope.getAttributes()
              == null) { // if no specific attributes have been defined, then find all attributes
            targetAttributes = new ArrayList<String>();

            MBeanInfo info = mbs.getMBeanInfo(name);
            for (MBeanAttributeInfo attributeInfo :
                info.getAttributes()) { // iterate through each attribute under the mBean
        ...
        if (information.size() + entriesFromScope.size() <= maxEntryCount) {
          ...
          information.putAll(entriesFromScope);
          logger.warn(
              "Stop collecting JXM entries as the max entries [" + maxEntryCount + "] has reached");
        } else {
          logger.warn(
              "Stop collecting JXM entries as the max entries [" + maxEntryCount + "] has reached");
          break;
   ...
   */
  private static Map<MetricKey, Number> processMBeanAttribute(
      ObjectName mBeanName, String attributeName, MBeanServer mbs) throws Exception {
    String prefix =
        (JMX_LABEL + "." + getMetricNameSegmentFromObjectName(mBeanName) + "." + attributeName)
            .replace(' ', '_');

    AttributeProcessor processor =
        AttributeProcessorLocator.getProcessor(mBeanName, attributeName, mbs);
    ...
    for (Entry<String, Object> entry : processedData.entrySet()) {
      ...
      if (value instanceof Number) {
        ...
        logger.debug(
            "Skipping JMX entry ["
                + metricName
                + "] in metrics reporting, as its value ["
                + value
                + "] is not a number!");
      }
    }
  ...
  public List<? extends MetricsEntry<?>> collectMetricsEntries()
    ...
    List<SimpleMeasurementMetricsEntry> metricsEntries =
