...
/**
 * Base class of processing an attribute. Processors are responsible to process the JMX attribute
 * and convert it into a map filled with keys and values.
 *
 * <p>The concrete child class implements the actual logic and behavior of the processing
 *
 * @author Patson Luk
 */
public abstract class AttributeProcessor {
  ...
  /**
   * Appends an attribute object to the input container map, take note that the object will be
   * flattened if it is of type <code>CompositeData</code>, <code>TabularData</code> or arrays.
   *
   * <p>Otherwise the data would be converted to its String value and stored into the input
   * container map
   *
  ...
  protected void appendAttribute(Object attributeObj, String prefix, Map<String, Object> values) {
    ...
    if (attributeObj == null) {
      ...
      // for example, if the index is "PS Survivor Space", then the full entry would be something
      // like
      ...
      // for array data, we will insert the key as [arrayIndex] for example Threading.thread[2].id =
      // 2
      for (int i = 0;
          i < Array.getLength(attributeObj);
          i++) { // We cannot cast directly to Object[] as it would fail on primitive arrays
  ...
  private void handleCompositeData(
      CompositeData compositeData, String prefix, Map<String, Object> values) {
   ...
   */
  public abstract Map<String, Object> process(ObjectName mBeanName, String attributeName)
      throws Exception;
}
