 ...
 *
 * <p>We are using the MANIFEST.MF within the jar for framework information. Take note that if the
 * information in the MANIFEST does not comply to the spec in <a
 * href="http://docs.oracle.com/javase/7/docs/technotes/guides/jar/jar.html">...</a> , then we will
 * not report the framework
 *
 * @author Patson Luk
 */
public class FrameworkRecorder {
  private static final Logger logger = LoggerFactory.getLogger();
  private static final VersionExtractor[] EXTRACTORS =
      new VersionExtractor[] {
        new BundleVersionExtractor(),
        new ImplementationVersionExtractor(),
        new SpecificationVersionExtractor()
      };
  private static final Set<FrameworkInfo> pendingFrameworks = new HashSet<FrameworkInfo>();
  private static final Map<URL, Boolean> processedSources =
      Collections.synchronizedMap(
          new WeakHashMap()); // synchronize it as it might hang indefinitely sometimes on docker

  // instance, could be related to
   ...
   */
  public static void reportFramework(
      ProtectionDomain protectionDomain, ClassLoader classLoader, String packageName) {
    // skip core java classes as it might introduce deadlock, also the java version should have
    // already been reported in init message
    if (packageName != null
      ...
      if (protectionDomain.getCodeSource() != null) {
        ...
        if (location != null && !processedSources.containsKey(location)) {
          ...
          if (frameworkInfo == null && packageName != null) {
            logger.debug(
                "Failed to extract framework info from source "
                    + location
                    + " try to load framework info using Package instead "
                    + packageName);
  ...
  private static FrameworkInfo extractFrameworkInfoFromSource(URL sourceLocation) {
    ...
    try {
      ...
      if (sourceStream != null) { // source stream could be null, for example
        // file:/C:/apache-tomcat-9.0.30/webapps/test_jdbc_war/WEB-INF/classes/
        try (JarInputStream jarInputStream = new JarInputStream(sourceStream)) {
          ...
          if (manifest != null) {
            ...
            if (frameworkInfo != null) {
              logger.debug(
                  "Framework info extracted from [" + sourceLocation + "] : " + frameworkInfo);
            } else {
      ...
      logger.warn(
          "Unexpected exception while parsing source file ["
              + sourceLocation
              + "] for framework versions, the entry would be skipped : "
              + e.getMessage(),
          e);
    } finally {
      if (sourceStream != null) {
        try {
          ...
          logger.warn(
              "Failed to close source stream after framework extraction: " + e.getMessage(), e);
        }
      }
    }
    return null;
    ...
    if (packageInfo != null) {
      if (packageInfo.getImplementationTitle() != null) { // have to at least have title
        return new FrameworkInfo(
            packageInfo.getImplementationTitle(), packageInfo.getImplementationVersion());
      } else if (packageInfo.getImplementationVendor()
          != null) { // if implementation title is not found, use vendor name as title (this works
        // for Jetty)
        return new FrameworkInfo(
            packageInfo.getImplementationVendor(), packageInfo.getImplementationVersion());
      } else if (packageInfo.getSpecificationTitle() != null) {
        return new FrameworkInfo(
            packageInfo.getSpecificationTitle(), packageInfo.getSpecificationVersion());
      } else if (packageInfo.getSpecificationVendor() != null) {
        return new FrameworkInfo(
            packageInfo.getSpecificationVendor(), packageInfo.getSpecificationVersion());
      }
    }
    return null;
  }
  ...
  /**
   * Get clone of the list of pending frameworks, this would also clear the existing pending
   * frameworks
   *
   * @return
   */
  ...
  private abstract static class VersionExtractor {
          ...
          if (value != null) {
            logger.debug(
                "Found ["
                    ...
                    + "] with value ["
                    + value
                    + "] while extracting framework version ["
                    + version
                    + "] from "
  ...
  private static class ImplementationVersionExtractor extends VersionExtractor {
    ...
    protected String[] getIdentityKeys() {
      return new String[] {
        "Implementation-Vendor-Id", "Implementation-Title", "Implementation-Vendor"
      };
    }
