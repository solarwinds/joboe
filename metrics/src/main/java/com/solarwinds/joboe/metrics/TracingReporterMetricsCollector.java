...
/**
 * Sub metrics collector that collects metrics specific to the Tracing event reporter such as number
 * of tracing events sent, the largest queue size since last collection etc
 *
 * @author pluk
 */
...
public class TracingReporterMetricsCollector extends AbstractMetricsEntryCollector {
  ...
  public List<SimpleMeasurementMetricsEntry> collectMetricsEntries() throws Exception {
    ...
    metricsEntries.add(
        new SimpleMeasurementMetricsEntry("NumOverflowed", stats.getOverflowedCount()));
    ...
    metricsEntries.add(
        new SimpleMeasurementMetricsEntry("QueueLargest", stats.getQueueLargestCount()));
