 ...
 *
 * <p>Manages a list of {@code SystemMonitor} for system monitoring. Take note that this controller
 * can only be started once per JVM
 *
 * <p>Each {@code SystemMonitor} is a self contained Runnable with it's own interval, the behavior
 * is implemented in {@link SystemMonitor#run()}. The {@code SystemMonitor} defines {@link
 * SystemCollector} and {@link SystemReporter} in order to collect and report data. For detailed
 * information, please refer to documentation in {@link SystemMonitor}.
 *
 * <p>The construction of the monitors is implemented in {@link SystemMonitorFactoryImpl}. Therefore
 * to attach extra {@code SystemMonitor}, add the code logic in that factory.
 *
 * @see SystemMonitor
 * @author Patson Luk
 */
public class SystemMonitorController {
  ...
  /**
   * Starts the System monitoring. Should only be called once. If more than once has been called,
   * the other calls would be ignored
   */
