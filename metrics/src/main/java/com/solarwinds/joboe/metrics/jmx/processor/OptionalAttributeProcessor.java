...
/**
 * A CPU time attribute processor that is more forgiving when the attribute is missing. Such an
 * attribute is not found in IBM 1.5 jre
 *
 * @author Patson Luk
 */
class OptionalAttributeProcessor extends AttributeProcessor {
  ...
  @Override
  /**
   * Attempt to retrieve the CPU time information from MBeanServer. If not available, simply display
    ...
    try {
      ...
      logger.debug(
          "Cannot load attribute ["
              ...
              + "]. This is expected as some JVMs might not publish this attribute. Skipping...");
    }
