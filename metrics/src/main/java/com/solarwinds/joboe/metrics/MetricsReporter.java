...
import com.solarwinds.joboe.core.metrics.MetricsEntry.MetricsEntryType;
...
/**
 * {@code SystemReporter} for all metrics by converting metrics collected from {@link
 * MetricsCollector} into metrics messages, and send them using the provided rpc client
 *
 * <p>Take note that though <code>MetricsCollector</code> extracts metrics from several different
 * sources, this reporter handles those metrics generically without making any special cases based
 * on the source type.
 *
 * @author Patson Luk
 */
class MetricsReporter extends SystemReporter<MetricsCategory, List<? extends MetricsEntry<?>>> {
  ...
  private final ClientLoggingCallback<Result> loggingCallback =
      new ClientLoggingCallback<Result>("post metrics");
  ...
  public void reportData(
      Map<MetricsCategory, List<? extends MetricsEntry<?>>> collectedData, long interval)
    ...
    SortedMap<MetricsCategory, List<? extends MetricsEntry<?>>> sortedData =
        new TreeMap<MetricsCategory, List<? extends MetricsEntry<?>>>(collectedData);

    int measurementCount = 0;
    int histogramCount = 0;
    for (Entry<MetricsCategory, List<? extends MetricsEntry<?>>> collectedByMetricsType :
        sortedData.entrySet()) {
      for (MetricsEntry<?> metricsEntry : collectedByMetricsType.getValue()) {
        if (metricsEntry.getType() == MetricsEntryType.MEASUREMENT
            || metricsEntry.getType() == MetricsEntryType.HISTOGRAM) {
          if (metricsEntry.getType() == MetricsEntryType.MEASUREMENT) {
    ...
    if (measurementCount > MAX_MEASUREMENT_ENTRY_COUNT) {
      logger.warn(
          "Dropped "
              + (measurementCount - MAX_MEASUREMENT_ENTRY_COUNT)
              + " measurement entries as the limit "
              + MAX_MEASUREMENT_ENTRY_COUNT
              + " was exceeded");
    } else {
  ...
  private Map<String, Object> extractKeyValues(MetricsEntry<?> metricsEntry) {
    ...
    String trimmedMetricName =
        metricsEntry.getName().length() <= MAX_METRIC_NAME_LENGTH
            ? metricsEntry.getName()
            : metricsEntry.getName().substring(0, MAX_METRIC_NAME_LENGTH);

    extractedKeyValues.put("name", trimmedMetricName);
    if (metricsEntry.getTags() != null
        && !metricsEntry.getTags().isEmpty()) { // only add tags KV if tags is non-empty
      Map<String, String> trimmedTags =
          new HashMap<String, String>(); // AO metrics only support String value for now
      for (Entry<String, ?> tagEntry : metricsEntry.getTags().entrySet()) {
        String trimmedKey =
            tagEntry.getKey().length() <= MAX_TAG_NAME_LENGTH
                ? tagEntry.getKey()
                : tagEntry.getKey().substring(0, MAX_TAG_NAME_LENGTH);
        ...
        if (value != null) {
          ...
          if (value instanceof String) {
            ...
            trimmedValue =
                valueString.length() <= MAX_TAG_VALUE_LENGTH
                    ? valueString
                    : valueString.substring(0, MAX_TAG_VALUE_LENGTH);
          } else {
            trimmedValue = value;
          }
          ...
          logger.warn(
              "Unexpected null tag value for metrics ["
                  + metricsEntry.getName()
                  + "] with tag ["
                  + tagEntry.getKey()
                  + "]");
  ...
  private void putInfoKeyValues(Map<String, Object> metricsMessage, int flushInterval) {
    ...
    metricsMessage.put("Timestamp_u", TimeUtils.getTimestampMicroSeconds());
