...
/**
 * {@code SystemMonitor} for various metrics. The the list of supported metrics, please refer to
 * {@link MetricsCollector}
 *
 * @author Patson Luk
 */
public class MetricsMonitor
    extends SystemMonitorWithFrequency<MetricsCategory, List<? extends MetricsEntry<?>>> {
  ...
  public MetricsMonitor(ConfigContainer configs, MetricsCollector metricsCollector)
      throws InvalidConfigException, ClientException {
    super(
        DEFAULT_TIME_UNIT,
        DEFAULT_FREQUENCY,
        metricsCollector,
        new MetricsReporter(RpcClientManager.getClient(OperationType.METRICS)));
    ...
    SettingsManager.registerListener(
        new SettingsArgChangeListener<Integer>(SettingsArg.METRIC_FLUSH_INTERVAL) {
          @Override
  ...
  public static synchronized MetricsMonitor buildInstance(ConfigContainer configs)
      throws InvalidConfigException, ClientException {
    return buildInstance(configs, null);
  }

  public static synchronized MetricsMonitor buildInstance(
      ConfigContainer configs, MetricsCollector explicitCollector)
      throws InvalidConfigException, ClientException {
    if (singleton == null) {
      singleton =
          new MetricsMonitor(
              configs,
              explicitCollector != null ? explicitCollector : new MetricsCollector(configs));
    }
    return singleton;
  }
