 ...
 * expects {@link com.tracelytics.ext.apache.http.auth.AuthScheme} to be {@link java.io.Serializable}
        ...
        if (authScheme instanceof Serializable) {
            try {
                ...
                if (log.isLoggable(Level.WARNING)) {
                    log.log(Level.WARNING, "Unexpected I/O error while serializing auth scheme", ex);
                }
            }
        } else {
            if (log.isLoggable(Level.FINE)) {
                log.log(Level.FINE, "Auth scheme " + authScheme.getClass() + " is not serializable");
            }
        }
    }
    ...
    public AuthScheme get(final HttpHost host) {
        ...
        if (bytes != null) {
            try {
                ...
                if (log.isLoggable(Level.WARNING)) {
                    log.log(Level.WARNING, "Unexpected I/O error while de-serializing auth scheme", ex);
                }
                return null;
            } catch (ClassNotFoundException ex) {
                if (log.isLoggable(Level.WARNING)) {
                    log.log(Level.WARNING, "Unexpected error while de-serializing auth scheme", ex);
                }
                return null;
            }
        } else {
