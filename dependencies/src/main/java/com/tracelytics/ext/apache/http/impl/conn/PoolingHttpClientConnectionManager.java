...
import com.tracelytics.ext.apache.http.config.Registry;
import com.tracelytics.ext.apache.http.config.RegistryBuilder;
import com.tracelytics.ext.apache.http.config.SocketConfig;
import com.tracelytics.ext.apache.http.conn.ConnectionPoolTimeoutException;
import com.tracelytics.ext.apache.http.conn.ConnectionRequest;
import com.tracelytics.ext.apache.http.conn.DnsResolver;
import com.tracelytics.ext.apache.http.conn.HttpClientConnectionManager;
import com.tracelytics.ext.apache.http.conn.HttpClientConnectionOperator;
...
import com.tracelytics.ext.apache.http.pool.ConnFactory;
import com.tracelytics.ext.apache.http.pool.ConnPoolControl;
import com.tracelytics.ext.apache.http.pool.PoolStats;
...
public class PoolingHttpClientConnectionManager
    ...
    public ConnectionRequest requestConnection(
        ...
        if (this.log.isLoggable(Level.FINE)) {
            this.log.log(Level.FINE, "Connection request: " + format(route, state) + formatStats(route));
        }
    ...
    protected HttpClientConnection leaseConnection(
        ...
        try {
            ...
            if (this.log.isLoggable(Level.FINE)) {
                this.log.log(Level.FINE, "Connection leased: " + format(entry) + formatStats(entry.getRoute()));
            }
            ...
            try {
                if (conn.isOpen()) {
                    ...
                    if (this.log.isLoggable(Level.FINE)) {
                        ...
                        this.log.log(Level.FINE, "Connection " + format(entry) + " can be kept alive " + s);
                    }
                }
            } finally {
                this.pool.release(entry, conn.isOpen() && entry.isRouteComplete());
                if (this.log.isLoggable(Level.FINE)) {
                    this.log.log(Level.FINE, "Connection released: " + format(entry) + formatStats(entry.getRoute()));
                }
            }
        }
    }
    ...
    public void shutdown() {
        if (this.isShutDown.compareAndSet(false, true)) {
            this.log.log(Level.FINE, "Connection manager is shutting down");
            try {
                this.pool.shutdown();
            } catch (final IOException ex) {
                this.log.log(Level.FINE, "I/O exception shutting down connection manager", ex);
            }
            this.log.log(Level.FINE, "Connection manager shut down");
        }
    }
    ...
    public void closeIdleConnections(final long idleTimeout, final TimeUnit tunit) {
        if (this.log.isLoggable(Level.FINE)) {
            this.log.log(Level.FINE, "Closing connections idle longer than " + idleTimeout + " " + tunit);
        }
    ...
    public void closeExpiredConnections() {
        this.log.log(Level.FINE, "Closing expired connections");
