...
import com.tracelytics.ext.apache.http.client.NonRepeatableRequestException;
...
import com.tracelytics.ext.apache.http.conn.routing.BasicRouteDirector;
import com.tracelytics.ext.apache.http.conn.routing.HttpRoute;
import com.tracelytics.ext.apache.http.conn.routing.HttpRouteDirector;
import com.tracelytics.ext.apache.http.conn.routing.RouteTracker;
...
import com.tracelytics.ext.apache.http.impl.conn.ConnectionShutdownException;
    ...
    public CloseableHttpResponse execute(
        ...
        if (config.isStaleConnectionCheckEnabled()) {
            ...
            if (managedConn.isOpen()) {
                this.log.log(Level.FINE, "Stale connection check");
                if (managedConn.isStale()) {
                    this.log.log(Level.FINE, "Stale connection detected");
                    managedConn.close();
                }
            }
        }
        ...
        try {
            ...
            for (int execCount = 1;; execCount++) {
                ...
                if (!managedConn.isOpen()) {
                    this.log.log(Level.FINE, "Opening connection " + route);
                    try {
                        establishRoute(proxyAuthState, managedConn, route, request, context);
                    } catch (final TunnelRefusedException ex) {
                        if (this.log.isLoggable(Level.FINE)) {
                            this.log.log(Level.FINE, ex.getMessage());
                        }
                ...
                if (this.log.isLoggable(Level.FINE)) {
                    this.log.log(Level.FINE, "Executing request " + request.getRequestLine());
                }

                if (!request.containsHeader(AUTH.WWW_AUTH_RESP)) {
                    if (this.log.isLoggable(Level.FINE)) {
                        this.log.log(Level.FINE, "Target auth state: " + targetAuthState.getState());
                    }
                    this.authenticator.generateAuthResponse(request, targetAuthState, context);
                }
                if (!request.containsHeader(AUTH.PROXY_AUTH_RESP) && !route.isTunnelled()) {
                    if (this.log.isLoggable(Level.FINE)) {
                        this.log.log(Level.FINE, "Proxy auth state: " + proxyAuthState.getState());
                    }
                ...
                if (reuseStrategy.keepAlive(response, context)) {
                    ...
                    if (this.log.isLoggable(Level.FINE)) {
                        ...
                        this.log.log(Level.FINE, "Connection can be kept alive " + s);
                    }
    ...
    void establishRoute(
        ...
        do {
            ...
            case HttpRouteDirector.TUNNEL_TARGET: {
                ...
                this.log.log(Level.FINE, "Tunnel to target created.");
            ...
            case HttpRouteDirector.TUNNEL_PROXY: {
                ...
                this.log.log(Level.FINE, "Tunnel to proxy created.");
                ...
                if (this.authenticator.isAuthenticationRequested(proxy, response,
                    ...
                    if (this.authenticator.handleAuthChallenge(proxy, response,
                        ...
                        if (this.reuseStrategy.keepAlive(response, context)) {
                            this.log.log(Level.FINE, "Connection kept alive");
