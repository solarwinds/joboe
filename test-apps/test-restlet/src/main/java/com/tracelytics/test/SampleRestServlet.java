...
public class SampleRestServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            client.post(form);
            client.release();
            client.post(client.toRepresentation(form));
            client.release();
            ...
            try {
                client.patch(form);
                client.release();
                client.patch(client.toRepresentation(form));
                client.release();
                client.patch(form, String.class);
                client.release();
                client.patch(form, MediaType.APPLICATION_ALL);
                client.release();
            } catch (ResourceException e) {
                System.out.println(e.getStatus() + " - " + e.getMessage());
                System.out.println("This is expected as HttpUrlConnection / Http server might not support PATCH operations");
            }
            
            client.put(form);
            client.release();
            client.put(client.toRepresentation(form));
            client.release();
