...
class DemoServiceActor extends Actor with DemoService {
  implicit def myExceptionHandler = 
    ExceptionHandler {
    case e : ArithmeticException => context => {
      context.complete(200, "Bad numbers, bad result!!!")
    }
  }
...
trait DemoService extends HttpService {
  ...
  lazy val akkaController = actorRefFactory.actorOf(Props[AkkaController], "akka-controller")
  lazy val httpClientController = actorRefFactory.actorOf(Props[HttpClientController], "http-client-controller")
  ...
  val demoRoute = {
    get {
      ...
      path("ping") {
        parameter('echo ?) { 
          case Some(echoParam) => complete("PONG! echo " + echoParam)
          case None => complete("PONG!")
        }
      } ~
      path("twirl") { 
        complete(views.html.index("From twirl"))
      } ~
      path("stream1") {
      ...
      pathPrefix("http") { ctx =>
          println("http!")
          httpClientController ! ctx
      }
    } ~
    post {
      path("something") {
        complete("Posted Something!")
      }
    } ~
...
<li><a href="/ping?echo='hi'">/ping with echo param hi</a></li>
					<li><a href="/twirl">/twirl</a> (Twirl rendering)</li>
          ...
          <li><a href="/stop?method=post">/stop</a></li>
				  <li><a href="/akka/work">/akka/work (send work to a Akk actor controller that signals work to a Akka master actor which delegates work to child actor via a router</a></li>
					<li><a href="/http/google"> call to Google API via spray-client (not supported for now)</a></li>
					<li><a href="/http/cross-host-spray"> call to local 8081 (the dummy app) via spray-client (not supported for now)</a></li>
					<li><a href="/http/cross-host-plain"> call to local 8081 (the dummy app) via plain HttpURLConnection (should see traces from both hosts - 8080 and 8081)</a></li>
					<li><form action="/something" method="POST"><input type="submit" value="post something!"/></form></li>
        </ul>
      </body>
    </html>
