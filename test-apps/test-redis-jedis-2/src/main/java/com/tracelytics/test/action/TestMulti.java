...
//        transaction.getrange("string-string-key", 0, 1);
        ...
        transaction.linsert(LIST_BYTE_KEY, Client.LIST_POSITION.BEFORE, BYTE_VALUE, BYTE_VALUE);
        transaction.linsert(LIST_STRING_KEY, Client.LIST_POSITION.BEFORE, STRING_VALUE, STRING_VALUE);
...
//        jedis.pubsubNumSub("test-channel"); //not instrumenting publisher/subscriber
        ...
        transaction.set(BYTE_KEY, BYTE_VALUE, "NX".getBytes());
        transaction.set(STRING_KEY, STRING_VALUE, "NX");
        transaction.set(BYTE_KEY, BYTE_VALUE, "NX".getBytes(), "EX".getBytes(), EXPIRY);
        transaction.set(STRING_KEY, STRING_VALUE, "NX", "EX", EXPIRY);
        ...
        transaction = jedisO.multi();
        transaction.set(STRING_KEY, STRING_VALUE);
        transaction.migrate(REDIS_HOST, REDIS_PORT, STRING_KEY, 0, Protocol.DEFAULT_TIMEOUT);
