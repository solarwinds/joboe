...
public class TestRemove extends AbstractEhcacheAction {
    ...
    protected String test(Ehcache cache) {
        ...
        cache.removeAll(Arrays.asList(0, 1, 2, 999));
        
        cache.removeAll(Arrays.asList(0, 1, 2, 999), true);
        
        cache.removeElement(new Element(1, new Person(1, "Added", "Test")));
        
        cache.removeQuiet(NON_SERIALIZABLE_KEY);
        
        cache.removeQuiet(1);
        
        cache.removeWithWriter(1);
