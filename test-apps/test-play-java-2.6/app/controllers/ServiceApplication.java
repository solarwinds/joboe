...
public class ServiceApplication extends Controller {
    ...
    public CompletionStage<Result> getStream() {
     // Make the request
        CompletionStage<WSResponse> futureResponse = ws.url(REST_SERVER_URL + "?stream=true").setMethod("GET").stream();
        ...
        CompletionStage<Result> result = futureResponse.thenApply(response -> {
            ...
            if (response.getStatus() == 200) {
                ...
                String contentType =
                        Optional.ofNullable(responseHeaders.get("Content-Type"))
                                .map(contentTypes -> contentTypes.get(0))
                                .orElse("application/octet-stream");

                // If there's a content length, send that, otherwise return the body chunked
                Optional<String> contentLength = Optional.ofNullable(responseHeaders.get("Content-Length")).map(contentLengths -> contentLengths.get(0));
