...
public class TestUseKeyspace extends AbstractCqlAction {
    @Override
    protected String test(Session session) {
        String useStatement = "USE " + TEST_KEYSPACE;
        ResultSet set = session.execute(useStatement);
        
        printToOutput(useStatement, set);
