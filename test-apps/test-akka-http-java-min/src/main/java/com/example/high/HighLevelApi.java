...
public class HighLevelApi extends AllDirectives {
    ...
    private Route createRoute() {
        ...
        return handleExceptions(myExceptionHandler, () -> route(
                path("hello", () ->
                        get(() ->
                        ...
                        parameterOptional("host", host ->
                                parameterOptional("port", port ->
                                        parameterOptional("method", methodKey -> {
                                            ...
                                            CompletionStage<HttpResponse> future1 = Http.get(system).singleRequest(HttpRequest.create(url).withMethod(method), materializer);
                                            CompletionStage<HttpResponse> future2 = Http.get(system).singleRequest(HttpRequest.create(url).withMethod(method), materializer);
