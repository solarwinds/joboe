...
//    }
  ...
  def postTest = Action.async {
    val testServerUrl = "http://localhost:8080/test-rest-server/"
    
    val request = WS.url(testServerUrl).withMethod("POST")
    
    println(request.method)
    
    request.execute().map { wsResponse =>
       Ok("Done . " + wsResponse.status)
    }
  }
   ...
   def deleteTest = Action.async {
    ...
    val request = WS.url(testServerUrl).withMethod("DELETE")
