  ...
  def bind(host : String, port : Int)(implicit system : ActorSystem, materializer : ActorMaterializer, executionContext : ExecutionContext) : Future[ServerBinding] = {
    val route =
      path("hello") {
        get {
          extractRequest
//          val originalPromise = Promise[String]()
//          originalPromise.future.transform { tryS =>
//            println("Sleeping in extra future " + tryS)
//            Thread.sleep(1000)
//            println("Done sleeping in extra future")
//            Success("done")
//          }
//
//          originalPromise.success("hey")
//          Thread.sleep(20)
//          println(Await.result(originalPromise.future, Duration.Inf))
      ...
      path("cross-server") {
        get {
          parameters('host.?, 'port.?, 'path.?, 'method.?, 'isRelative.?) { (hostO, portO, pathO, methodO, isRelaiveO) =>
            ...
            val method = methodO.fold(HttpMethods.GET)(methodKey => HttpMethods.getForKeyCaseInsensitive(methodKey).getOrElse(HttpMethods.GET))
