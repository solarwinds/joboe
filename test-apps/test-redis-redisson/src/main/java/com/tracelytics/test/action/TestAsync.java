...
public class TestAsync extends AbstractRedissonAction {
    @Override
    protected String test() throws Exception {
        RedisClient client = new RedisClient(new NioEventLoopGroup(), REDIS_HOST);
...
//        connection.sortStore("random-list", SortArgs.Builder.asc(), "new-list");
        ...
        connection.configSet("timeout", connection.configGet("timeout").get().get(1));
        ...
        connection.restore(KEY_2, EXPIRY, connection.dump(KEY).get());
        
        connection.echo("hi");
        
        connection.eval("return { KEYS[1] }", ScriptOutputType.VALUE, new String[] {"1"});

        String stringSha = connection.scriptLoad("return {}").get();
        connection.evalsha(stringSha, ScriptOutputType.VALUE, new String[] {"1"});
        ...
        connection.pfadd(LOG_KEY, VALUE);
        connection.pfcount(LOG_KEY, LOG_KEY);
        connection.pfmerge(LOG_KEY, LOG_KEY, LOG_KEY);
        connection.ping();
        connection.psetex(KEY, EXPIRY, VALUE);
        connection.pttl(KEY);
