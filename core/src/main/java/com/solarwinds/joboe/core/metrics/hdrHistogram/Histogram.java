 ...
 *
 * <p>{@link Histogram} supports the recording and analyzing sampled data value counts across a
 ...
 * one second, and a resolution of 1 second (or better) up to 1,000 seconds. At its maximum tracked
 * value (1 hour), it would still maintain a resolution of 3.6 seconds (or better).
 *
 * <p>Histogram tracks value counts in <b><code>long</code></b> fields. Smaller field types are
 * available in the {@link IntCountsHistogram} and {@link ShortCountsHistogram} implementations of
...
public class Histogram extends AbstractHistogram {
  ...
  @Override
  public Histogram copyCorrectedForCoordinatedOmission(
  ...
  void resize(long newHighestTrackableValue) {
    ...
    if (oldNormalizedZeroIndex != 0) {
      ...
      System.arraycopy(
          counts, oldNormalizedZeroIndex, counts, newNormalizedZeroIndex, lengthToCopy);
  ...
  /**
   * Construct a Histogram given the Highest value to be tracked and a number of significant decimal
   * digits. The histogram will be constructed to implicitly track (distinguish from 0) values as
   * low as 1.
   *
   ...
   * source's start/end timestamps (but NOT its contents)
   *
   ...
   */
  public static Histogram decodeFromByteBuffer(
   ...
   */
  public static Histogram decodeFromCompressedByteBuffer(
      final ByteBuffer buffer, final long minBarForHighestTrackableValue)
