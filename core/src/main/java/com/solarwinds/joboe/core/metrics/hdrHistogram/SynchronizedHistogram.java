 ...
 *
 * <p>A {@link SynchronizedHistogram} is a variant of {@link Histogram} that is synchronized as a
 * whole, such that queries, copying, and addition operations are atomic with relation to
 * modification on the {@link SynchronizedHistogram}, and such that external accessors (e.g.
 * iterations on the histogram data) that synchronize on the {@link SynchronizedHistogram} instance
 * can safely assume that no modifications to the histogram data occur within their synchronized
 * block.
 *
 * <p>It is important to note that synchronization can result in blocking recoding calls. If
 * non-blocking recoding operations are required, consider using {@link ConcurrentHistogram}, {@link
 * AtomicHistogram}, or (recommended) {@link Recorder} or {@link SingleWriterRecorder} which were
 * intended for concurrent operations.
 *
 * <p>See package description for {@link com.solarwinds.metrics.hdrHistogram} and {@link Histogram}
 * for more details.
 */
public class SynchronizedHistogram extends Histogram {
   ...
   */
  public static SynchronizedHistogram decodeFromByteBuffer(
      final ByteBuffer buffer, final long minBarForHighestTrackableValue) {
    return decodeFromByteBuffer(
        buffer, SynchronizedHistogram.class, minBarForHighestTrackableValue);
  }
   ...
   */
  public static SynchronizedHistogram decodeFromCompressedByteBuffer(
      final ByteBuffer buffer, final long minBarForHighestTrackableValue)
  ...
  @Override
  public synchronized void recordValueWithCount(final long value, final long count)
   ...
   */
  @Override
  public synchronized void recordValue(
      final long value, final long expectedIntervalBetweenValueSamples)
      throws ArrayIndexOutOfBoundsException {
  ...
  public void addWhileCorrectingForCoordinatedOmission(
      final AbstractHistogram fromHistogram, final long expectedIntervalBetweenValueSamples) {
  ...
  @Override
  public synchronized long getCountBetweenValues(final long lowValue, final long highValue)
      throws ArrayIndexOutOfBoundsException {
