 ...
 * separately from "hot" fields, in an attempt to force the JVM to place the (hot) fields commonly
 * used in the value recording code paths close together. Subclass boundaries tend to be strongly
 * control memory layout decisions in most practical JVM implementations, making this an effective
 * method for control filed grouping layout.
 */
abstract class AbstractHistogramBase extends EncodableHistogram {
  ...
  /**
   * Power-of-two length of linearly scaled array slots in the counts array. Long enough to hold the
   * first sequence of entries that must be distinguished by a single unit (determined by configured
   * precision).
   */
  int subBucketCount;
 ...
 *
 * <p>AbstractHistogram supports the recording and analyzing sampled data value counts across a
 ...
 * resolution of 1 microsecond up to 1 millisecond, a resolution of 1 millisecond (or better) up to
 * one second, and a resolution of 1 second (or better) up to 1,000 seconds. At it's maximum tracked
...
public abstract class AbstractHistogram extends AbstractHistogramBase implements Serializable {
  ...
  // Sub-classes will typically add a totalCount field and a counts array field, which will likely
  // be laid out
  ...
  /**
   * Set internally tracked maxValue to new value if new value is greater than current one. May be
   * overridden by subclasses for synchronization or atomicity purposes.
   *
   ...
   *
   * @param numberOfSignificantValueDigits The number of significant decimal digits to which the
   ...
   *
   * @param lowestDiscernibleValue The lowest value that can be discerned (distinguished from 0) by
  ...
  private void init(
     ...
     */
    final long largestValueWithSingleUnitResolution =
        2 * (long) Math.pow(10, numberOfSignificantValueDigits);

    unitMagnitude = (int) Math.floor(Math.log(lowestDiscernibleValue) / Math.log(2));
    unitMagnitudeMask = (1L << unitMagnitude) - 1;

    // We need to maintain power-of-two subBucketCount (for clean direct indexing) that is large
    // enough to
    // provide unit resolution to at least largestValueWithSingleUnitResolution. So figure out
    // largestValueWithSingleUnitResolution's nearest power-of-two (rounded up), and use that:
    int subBucketCountMagnitude =
        (int) Math.ceil(Math.log(largestValueWithSingleUnitResolution) / Math.log(2));
   ...
   * previous buckets put together). In other words, the k'th bucket could represent 0 * 2^k to 2048
   * * 2^k in 2048 buckets with 2^k precision, but the midpoint of 1024 * 2^k = 2048 * 2^(k-1) = the
   * k-1'th bucket's end, so we would use the previous bucket for those lower values as it has
   * better precision.
   */
  ...
  final int determineArrayLengthNeeded(long highestTrackableValue) {
    ...
    int countsArrayLength =
        getLengthForNumberOfBuckets(getBucketsNeededToCoverValue(highestTrackableValue));
  ...
  /**
   * Indicate whether or not the histogram is capable of supporting auto-resize functionality. Note
   * that this is an indication that enabling auto-resize by calling setAutoResize() is allowed, and
   * NOT that the histogram will actually auto-resize. Use isAutoResize() to determine if the
   * histogram is in auto-resize mode.
   *
   * @return autoResize setting
   */
   ...
   * method provided by {@link #copyCorrectedForCoordinatedOmission(long)}. The two methods are
   * mutually exclusive, and only one of the two should be be used on a given data set to correct
  ...
  private void handleRecordException(final long count, final long value, Exception ex) {
    if (!autoResize) {
      throw new ArrayIndexOutOfBoundsException(
          "value outside of histogram covered range. Caused by: " + ex);
    }
    resize(value);
    int countsIndex = countsArrayIndex(value);
    addToCountAtIndex(countsIndex, count);
  ...
  private void recordValueWithCountAndExpectedInterval(
      final long value, final long count, final long expectedIntervalBetweenValueSamples)
      throws ArrayIndexOutOfBoundsException {
    recordCountAtValue(count, value);
  ...
  private void recordSingleValueWithExpectedInterval(
      final long value, final long expectedIntervalBetweenValueSamples)
      throws ArrayIndexOutOfBoundsException {
    recordSingleValue(value);
    if (expectedIntervalBetweenValueSamples <= 0) return;
   ...
   * provided by {@link #recordValueWithExpectedInterval(long, long)
   ...
   */
  public abstract AbstractHistogram copyCorrectedForCoordinatedOmission(
  ...
  public void copyIntoCorrectedForCoordinatedOmission(
      final AbstractHistogram targetHistogram, final long expectedIntervalBetweenValueSamples) {
    targetHistogram.reset();
   ...
   *
   * <p>As part of adding the contents, the start/end timestamp range of this histogram will be
   * extended to include the start/end timestamp range of the other histogram.
   *
   * @param otherHistogram The other histogram.
   * @throws ArrayIndexOutOfBoundsException (may throw) if values in fromHistogram's are higher than
   *     highestTrackableValue.
   */
   ...
   *
   * <p>The start/end timestamps of this histogram will remain unchanged.
   *
   * @param otherHistogram The other histogram.
   * @throws ArrayIndexOutOfBoundsException (may throw) if values in otherHistogram's are higher
   ...
   * method provided by {@link #recordValueWithExpectedInterval(long, long)
   * recordValueWithExpectedInterval}. The two methods are mutually exclusive, and only one of the
   * two should be be used on a given data set to correct for the same coordinated omission issue.
   * by
   *
   ...
   * corrective behavior is important.
   *
   * @param otherHistogram The other histogram. highestTrackableValue and
   *     largestValueWithSingleUnitResolution must match.
   * @param expectedIntervalBetweenValueSamples If expectedIntervalBetweenValueSamples is larger
   *     than 0, add auto-generated value records as appropriate if value is larger than
   *     expectedIntervalBetweenValueSamples
  ...
  public void addWhileCorrectingForCoordinatedOmission(
      final AbstractHistogram otherHistogram, final long expectedIntervalBetweenValueSamples) {
  ...
  /**
   * Shift recorded values to the left (the equivalent of a &lt;&lt; shift operation on all recorded
   * values). The configured integer value range limits and value precision setting will remain
   * unchanged.
   *
   * <p>An {@link ArrayIndexOutOfBoundsException} will be thrown if any recorded values may be lost
   * as a result of the attempted operation, reflecting an "overflow" conditions. Expect such an
   * overflow exception if the operation would cause the current maxValue to be scaled to a value
   * that is outside of the covered value range.
   *
  ...
  /**
   * Shift recorded values to the right (the equivalent of a &gt;&gt; shift operation on all
   * recorded values). The configured integer value range limits and value precision setting will
   * remain unchanged.
   *
   * <p>Shift right operations that do not underflow are reversible with a shift left operation with
   * no loss of information. An {@link ArrayIndexOutOfBoundsException} reflecting an "underflow"
   * conditions will be thrown if any recorded values may lose representation accuracy as a result
   * of the attempted shift operation.
   *
   * <p>For a shift of a single order of magnitude, expect such an underflow exception if any
   * recorded non-zero values up to [numberOfSignificantValueDigits (rounded up to nearest power of
   * 2) multiplied by (2 ^ numberOfBinaryOrdersOfMagnitude) currently exist in the histogram.
   *
   * @param numberOfBinaryOrdersOfMagnitude The number of binary orders of magnitude to shift by
   */
  ...
  public long sizeOfEquivalentValueRange(final long value) {
    ...
    long distanceToNextValue =
        (1L
            << (unitMagnitude
                + ((subBucketIndex >= subBucketCount) ? (bucketIndex + 1) : bucketIndex)));
  ...
  /**
   * Get the lowest recorded non-zero value level in the histogram. If the histogram has no recorded
  ...
  public long getMinNonZeroValue() {
    return (minNonZeroValue == Long.MAX_VALUE)
        ? Long.MAX_VALUE
        : lowestEquivalentValue(minNonZeroValue);
  }
  ...
  public double getStdDeviation() {
    ...
    while (recordedValuesIterator.hasNext()) {
      ...
      geometric_deviation_total +=
          (deviation * deviation) * iterationValue.getCountAddedInThisIterationStep();
    }
  ...
  /**
   * Get the value at a given percentile. When the given percentile is &gt; 0.0, the value returned
   * is the value that the given percentage of the overall recorded value entries in the histogram
   * are either smaller than or equivalent to. When the given percentile is 0.0, the value returned
   * is the value that all value entries in the histogram are either larger than or equivalent to.
   *
   * <p>Note that two values are "equivalent" in this statement if {@link
   * AbstractHistogram#valuesAreEquivalent} would return true.
   *
  ...
  public long getValueAtPercentile(final double percentile) {
    ...
    long countAtPercentile =
        (long) (((requestedPercentile / 100.0) * getTotalCount()) + 0.5); // round to nearest
    countAtPercentile =
        Math.max(countAtPercentile, 1); // Make sure we at least reach the first recorded entry
   ...
   *     Will be rounded down with {@link Histogram#lowestEquivalentValue lowestEquivalentValue}.
   ...
   */
  public long getCountBetweenValues(final long lowValue, final long highValue)
      throws ArrayIndexOutOfBoundsException {
   ...
   * @return An {@link Iterable}{@literal <}{@link HistogramIterationValue}{@literal >} through the
  ...
  /**
   * An {@link Iterable}{@literal <}{@link HistogramIterationValue}{@literal >} through the
   * histogram using a {@link PercentileIterator}
   */
  public static class Percentiles implements Iterable<HistogramIterationValue> {
    ...
    private Percentiles(
        final AbstractHistogram histogram, final int percentileTicksPerHalfDistance) {
  ...
  public void outputPercentileDistribution(
    ...
    if (useCsvFormat) {
      ...
      lastLinePercentileFormatString =
          "%." + numberOfSignificantValueDigits + "f,%.12f,%d,Infinity\n";
    } else {
    ...
    if (!useCsvFormat) {
      ...
      printStream.format(
          ...
          getMaxValue() / outputValueUnitScalingRatio,
          getTotalCount());
      printStream.format(
          Locale.US, "#[Buckets = %12d, SubBuckets     = %12d]\n", bucketCount, subBucketCount);
    }
  }
  ...
  private void writeObject(final ObjectOutputStream o) throws IOException {
    ...
    // Max Value is added to the serialized form because establishing max via scanning is "harder"
    // during
    // deserialization, as the counts array is not available at the subclass deserializing level,
    // and we don't
  ...
  private void readObject(final ObjectInputStream o) throws IOException, ClassNotFoundException {
    ...
    // Set internalTrackingValues (can't establish them from array yet, because it's not yet
    // read...)
  ...
  private static final int V2maxWordSizeInBytes =
      9; // LEB128-64b9B + ZigZag require up to 9 bytes per word
  ...
  public synchronized int encodeIntoByteBuffer(final ByteBuffer buffer) {
    ...
    buffer.putInt(
        initialPosition + 4, buffer.position() - payloadStartPosition); // Record the payload length
  ...
  public synchronized int encodeIntoCompressedByteBuffer(
    ...
    if (intermediateUncompressedByteBuffer == null
        || intermediateUncompressedByteBuffer.capacity() < neededCapacity) {
    ...
    int compressedDataLength =
        compressor.deflate(
            targetArray, compressedTargetOffset, targetArray.length - compressedTargetOffset);
    compressor.end();
    ...
    targetBuffer.putInt(
        initialTargetPosition + 4, compressedDataLength); // Record the compressed length
  ...
  static <T extends AbstractHistogram> T decodeFromByteBuffer(
    ...
    try {
      ...
      histogram =
          constructor.newInstance(
              lowestTrackableUnitValue, highestTrackableValue, numberOfSignificantValueDigits);
    ...
    if (decompressor == null) {
      ...
      if (expectedCapacity > buffer.remaining()) {
        throw new IllegalArgumentException(
            "The buffer does not contain the full Histogram payload");
      }
      ...
      if ((payloadLengthInBytes != Integer.MAX_VALUE)
          && (decompressedByteCount < payloadLengthInBytes)) {
        throw new IllegalArgumentException(
            "The buffer does not contain the indicated payload amount");
      }
    }
    ...
    int filledLength =
        ((AbstractHistogram) histogram)
            .fillCountsArrayFromSourceBuffer(
                payLoadSourceBuffer, expectedCapacity, getWordSizeInBytesFromCookie(cookie));
  ...
  private int fillCountsArrayFromSourceBuffer(
      ByteBuffer sourceBuffer, int lengthInBytes, int wordSizeInBytes) {
    ...
    while (sourceBuffer.position() < endPosition) {
      ...
      if (wordSizeInBytes == V2maxWordSizeInBytes) {
        // V2 encoding format uses a long encoded in a ZigZag LEB128 format (up to
        // V2maxWordSizeInBytes):
        ...
        count =
            ((wordSizeInBytes == 2)
                ? sourceBuffer.getShort()
                : ((wordSizeInBytes == 4) ? sourceBuffer.getInt() : sourceBuffer.getLong()));
      }
      if (count > maxAllowableCountInHistigram) {
        throw new IllegalArgumentException(
            "An encoded count ("
                ...
                + this.wordSizeInBytes
                + " bytes) was encountered in the source");
      }
      if (zerosCount > 0) {
  ...
  int getBucketsNeededToCoverValue(final long value) {
    ...
    while (smallestUntrackableValue <= value) {
      if (smallestUntrackableValue > (Long.MAX_VALUE / 2)) {
        // next shift will overflow, meaning that bucket could represent values up to ones greater
        // than
  ...
  /**
   * If we have N such that subBucketCount * 2^N > max value, we need storage for N+1 buckets, each
   * with enough slots to hold the top half of the subBucketCount (the lower half is covered by
   * previous buckets), and the +1 being used for the lower half of the 0'th bucket. Or,
  ...
  private int countsArrayIndex(final int bucketIndex, final int subBucketIndex) {
    ...
    // subBucketCount).
    // For bucketIndex 0, all subBucketCount entries may be used, but bucketBaseIndex is still set
    // in the middle.
    final int bucketBaseIndex = (bucketIndex + 1) << subBucketHalfCountMagnitude;
    // Calculate the offset in the bucket. This subtraction will result in a positive value in all
    // buckets except
    // the 0th bucket (since a value in that bucket may be less than half the bucket's 0 to
    // subBucketCount range).
  ...
  int getBucketIndex(final long value) {
    // Calculates the number of powers of two by which the value is greater than the biggest value
    // that fits in
  ...
  int getSubBucketIndex(final long value, final int bucketIndex) {
    ...
    // For other bucketIndex, this will always end up in the top half of subBucketCount: assume that
    // for some bucket
    // k > 0, this calculation will yield a value in the bottom half of 0 to subBucketCount. Then,
    // because of how
    // buckets overlap, it would have also been in the top half of bucket k-1, and therefore would
    // have
    // returned k-1 in getBucketIndex(). Since we would then shift it one fewer bits here, it would
    // be twice as big,
  ...
  int normalizeIndex(int index, int normalizingIndexOffset, int arrayLength) {
    ...
    // The following is the same as an unsigned remainder operation, as long as no double wrapping
    // happens
    // (which shouldn't happen, as normalization is never supposed to wrap, since it would have
    // overflowed
