 ...
 *
 *
 * <h3>An integer values High Dynamic Range (HDR) Histogram that supports safe concurrent recording
 * operations.</h3>
 *
 ...
 * only thread-safe behaviors provided by {@link ConcurrentHistogram}, and that it is not otherwise
 * synchronized. Specifically, {@link ConcurrentHistogram} provides no implicit synchronization that
 * would prevent the contents of the histogram from changing during queries, iterations, copies, or
 * addition operations on the histogram. Callers wishing to make potentially concurrent,
 * multi-threaded updates that would safely work in the presence of queries, copies, or additions of
 * histogram objects should either take care to externally synchronize and/or order their access,
 * use the {@link SynchronizedHistogram} variant, or (recommended) use {@link Recorder} or {@link
 * SingleWriterRecorder} which are intended for this purpose.
 *
 * <p>Auto-resizing: When constructed with no specified value range range (or when auto-resize is
 * turned on with {@link Histogram#setAutoResize}) a {@link Histogram} will auto-resize its dynamic
 * range to include recorded values as they are encountered. Note that recording calls that cause
 * auto-resizing may take longer to execute, as resizing incurs allocation and copying of internal
 * data structures.
 *
...
public class ConcurrentHistogram extends Histogram {
  ...
  long getCountAtIndex(final int index) {
    try {
      ...
      long activeCount =
          activeCounts.get(
              normalizeIndex(
                  index, activeCounts.getNormalizingIndexOffset(), activeCounts.length()));
      long inactiveCount =
          inactiveCounts.get(
              normalizeIndex(
                  index, inactiveCounts.getNormalizingIndexOffset(), inactiveCounts.length()));
  ...
  private void shiftLowestInactiveHalfBucketContentsLeft(int shiftAmount) {
    ...
    for (int fromIndex = 1; fromIndex < subBucketHalfCount; fromIndex++) {
      ...
      int normalizedToIndex =
          normalizeIndex(
              toIndex, inactiveCounts.getNormalizingIndexOffset(), inactiveCounts.length());
    ...
    try {
      ...
      inactiveCounts =
          new AtomicLongArrayWithNormalizingOffset(
              newArrayLength, inactiveCounts.getNormalizingIndexOffset());
      ...
      // We resized things. We can now make the historam establish size accordingly for future
      // recordings:
  ...
  @Override
  public ConcurrentHistogram copyCorrectedForCoordinatedOmission(
   ...
   */
  public static ConcurrentHistogram decodeFromByteBuffer(
   ...
   */
  public static ConcurrentHistogram decodeFromCompressedByteBuffer(
      final ByteBuffer buffer, final long minBarForHighestTrackableValue)
      throws DataFormatException {
    return decodeFromCompressedByteBuffer(
        buffer, ConcurrentHistogram.class, minBarForHighestTrackableValue);
  }
