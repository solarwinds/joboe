 ...
 *
 * <p>A Histogram logs are used to capture full fidelity, per-time-interval histograms of a recorded
 * value.
 *
 ...
 *
 * <p>This log writer will produce histogram logs that adhere to the histogram log format (see
 * {{@link HistogramLogReader} for log format details). Optional comments, start time, legend, and
 * format version can be logged.
 *
 * <p>The log writer will use the
 *
 * <p>By convention, it is typical for the logging application to use a comment to indicate the
 * logging application at the head of the log, followed by the log format version, a start time, and
 * a legend (in that order).
 */
public class HistogramLogWriter {
  ...
  /**
   * -- GETTER -- return the current base time offset (see ).
   *
   * @return the current base time
   */
  @Getter private long baseTime = 0;
   ...
   * associated with the histogram, using a configurable maxValueUnitRatio. (note that the specified
   * timestamp information will be used, and the timestamp information in the actual histogram will
   * be ignored). The max value reported with the interval line will be scaled by the given
   * maxValueUnitRatio.
   *
   ...
   * @param maxValueUnitRatio The ratio by which to divide the histogram's max value when reporting
   *     on it.
   */
  public synchronized void outputIntervalHistogram(
      final double startTimeStampSec,
      final double endTimeStampSec,
      final EncodableHistogram histogram,
      final double maxValueUnitRatio) {
    if ((targetBuffer == null)
        || targetBuffer.capacity() < histogram.getNeededByteBufferCapacity()) {
      targetBuffer = ByteBuffer.allocate(histogram.getNeededByteBufferCapacity()).order(BIG_ENDIAN);
    }
    targetBuffer.clear();

    int compressedLength =
        histogram.encodeIntoCompressedByteBuffer(targetBuffer, Deflater.BEST_COMPRESSION);
    ...
    if (tag == null) {
      ...
      if (containsDelimeterMatcher.matches()) {
        throw new IllegalArgumentException(
            "Tag string cannot contain commas, spaces, or line breaks");
      }
      log.format(
          Locale.US,
          "Tag=%s,%.3f,%.3f,%.3f,%s\n",
  ...
  /**
   * Output an interval histogram, with the given timestamp information, and the [optional] tag
   * associated with the histogram. (note that the specified timestamp information will be used, and
   * the timestamp information in the actual histogram will be ignored). The max value in the
   * histogram will be reported scaled down by a default maxValueUnitRatio of 1,000,000 (which is
   * the msec : nsec ratio). Caller should use the direct form specifying maxValueUnitRatio some
   * other ratio is needed for the max value output.
   *
   * @param startTimeStampSec The start timestamp to log with the interval histogram, in seconds.
   * @param endTimeStampSec The end timestamp to log with the interval histogram, in seconds.
   * @param histogram The interval histogram to log.
   */
  public void outputIntervalHistogram(
      final double startTimeStampSec,
      final double endTimeStampSec,
      final EncodableHistogram histogram) {
  ...
  /**
   * Output an interval histogram, using the start/end timestamp indicated in the histogram, and the
   * [optional] tag associated with the histogram. The histogram start and end timestamps are
   * assumed to be in msec units. Logging will be in seconds, realtive by a base time (if set via
   * {@link HistogramLogWriter#setBaseTime}). The default base time is 0.
   *
   * <p>By covention, histogram start/end time are generally stamped with absolute times in msec
   * since the epoch. For logging with absolute time stamps, the base time would remain zero. For
   * logging with relative time stamps (time since a start point), the base time should be set with
   * {@link HistogramLogWriter#setBaseTime}.
   *
   * <p>The max value in the histogram will be reported scaled down by a default maxValueUnitRatio
   * of 1,000,000 (which is the msec : nsec ratio). Caller should use the direct form specifying
  ...
  public void outputIntervalHistogram(final EncodableHistogram histogram) {
    outputIntervalHistogram(
        (histogram.getStartTimeStamp() - baseTime) / 1000.0,
  ...
  /**
   * Set a base time to subtract from supplied histogram start/end timestamps when logging based on
   * histogram timestamps. Base time is expected to be in msec since the epoch, as histogram
   * start/end times are typically stamped with absolute times in msec since the epoch.
   *
