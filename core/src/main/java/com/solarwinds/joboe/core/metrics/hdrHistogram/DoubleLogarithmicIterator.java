...
/**
 * Used for iterating through {@link DoubleHistogram} values values in logarithmically increasing
 * levels. The iteration is performed in steps that start at <i>valueUnitsInFirstBucket</i> and
 * increase exponentially according to <i>logBase</i>, terminating when all recorded histogram
 * values are exhausted. Note that each iteration "bucket" includes values up to and including the
 * next bucket boundary value.
 */
public class DoubleLogarithmicIterator implements Iterator<DoubleHistogramIterationValue> {
  ...
  public void reset(final double valueUnitsInFirstBucket, final double logBase) {
    integerLogarithmicIterator.reset(
        (long) (valueUnitsInFirstBucket * histogram.doubleToIntegerValueConversionRatio), logBase);
  }
   ...
   * @param logBase the multiplier by which the bucket size is expanded in each iteration step.
   */
  public DoubleLogarithmicIterator(
      final DoubleHistogram histogram, final double valueUnitsInFirstBucket, final double logBase) {
    this.histogram = histogram;
    integerLogarithmicIterator =
        new LogarithmicIterator(
            histogram.integerValuesHistogram,
            (long) (valueUnitsInFirstBucket * histogram.doubleToIntegerValueConversionRatio),
            logBase);
    iterationValue =
        new DoubleHistogramIterationValue(integerLogarithmicIterator.currentIterationValue);
  }
