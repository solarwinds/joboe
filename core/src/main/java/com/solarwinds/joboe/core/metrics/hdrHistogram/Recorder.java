 ...
 * ongoing.
 *
 * <p>{@link Recorder} supports concurrent {@link Recorder#recordValue} or {@link
 * Recorder#recordValueWithExpectedInterval} calls. Recording calls are wait-free on architectures
 * that support atomic increment operations, and are lock-free on architectures that do not.
 */
public class Recorder {
  ...
  /**
   * Construct an auto-resizing {@link Recorder} with a lowest discernible value of 1 and an
  ...
  /**
   * Construct a {@link Recorder} given the Lowest and highest values to be tracked and a number of
   ...
   *
   * <p>Calling {@link Recorder#getIntervalHistogram()} will reset the value counts, and start
   ...
   *
   * <p>{@link Recorder#getIntervalHistogram(Histogram histogramToRecycle)
   ...
   * significantly more efficient for repeated use than {@link Recorder#getIntervalHistogram()} and
   * {@link Recorder#getIntervalHistogramInto getIntervalHistogramInto()}. The provided {@code
   * histogramToRecycle} must be either be null or an interval histogram returned by a previous call
   * to {@link Recorder#getIntervalHistogram(Histogram histogramToRecycle)
   ...
   * undefined.
   *
   * <p>Calling {@link Recorder#getIntervalHistogram(Histogram histogramToRecycle)
   ...
   *
   * <p>Calling {@link Recorder#getIntervalHistogramInto getIntervalHistogramInto()} will reset the
   * value counts, and start accumulating value counts for the next interval.
   *
    ...
    try {
      ...
      if (inactiveHistogram == null) {
        if (activeHistogram instanceof InternalAtomicHistogram) {
          ...
          inactiveHistogram =
              new InternalConcurrentHistogram(
  ...
  private static class InternalAtomicHistogram extends AtomicHistogram {
    ...
    private InternalAtomicHistogram(
        long id,
