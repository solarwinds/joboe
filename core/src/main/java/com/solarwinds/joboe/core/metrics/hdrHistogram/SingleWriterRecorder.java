...
/**
 * Records integer values, and provides stable interval {@link Histogram} samples from live recorded
 * data without interrupting or stalling active recording of values. Each interval histogram
 * provided contains all value counts accumulated since the previous interval histogram was taken.
 *
 * <p>This pattern is commonly used in logging interval histogram information while recording is
 * ongoing.
 *
 * <p>{@link SingleWriterRecorder} expects only a single thread (the "single writer") to call {@link
 * SingleWriterRecorder#recordValue} or {@link SingleWriterRecorder#recordValueWithExpectedInterval}
 * at any point in time. It DOES NOT safely support concurrent recording calls.
 */
public class SingleWriterRecorder {
  ...
  /**
   * Construct an auto-resizing {@link SingleWriterRecorder} with a lowest discernible value of 1
  ...
  /**
   * Construct a {@link SingleWriterRecorder} given the highest value to be tracked and a number of
   ...
   *     separation. Must be a non-negative integer between 0 and 5.
   */
  public SingleWriterRecorder(
      final long highestTrackableValue, final int numberOfSignificantValueDigits) {
    this(1, highestTrackableValue, numberOfSignificantValueDigits);
  }

  /**
   * Construct a {@link SingleWriterRecorder} given the Lowest and highest values to be tracked and
   * a number of significant decimal digits. Providing a lowestDiscernibleValue is useful is
   ...
   *
   * <p>See related notes {@link AbstractHistogram#recordValueWithExpectedInterval(long, long)} for
   * more explanations about coordinated omission and expected interval correction. *
   *
   * @param value The value to record
  ...
  /**
   * Get a new instance of an interval histogram, which will include a stable, consistent view of
   * all value counts accumulated since the last interval histogram was taken.
   *
   * <p>Calling {@link SingleWriterRecorder#getIntervalHistogram()} will reset the value counts, and
   * start accumulating value counts for the next interval.
   *
   * @return a histogram containing the value counts accumulated since the last interval histogram
   *     was taken.
   */
   ...
   *
   * <p>{@link SingleWriterRecorder#getIntervalHistogram(Histogram histogramToRecycle)
   * getIntervalHistogram(histogramToRecycle)} accepts a previously returned interval histogram that
   * can be recycled internally to avoid allocation and content copying operations, and is therefore
   * significantly more efficient for repeated use than {@link
   * SingleWriterRecorder#getIntervalHistogram()} and {@link
   * SingleWriterRecorder#getIntervalHistogramInto getIntervalHistogramInto()}. The provided {@code
   * histogramToRecycle} must be either be null or an interval histogram returned by a previous call
   * to {@link SingleWriterRecorder#getIntervalHistogram(Histogram histogramToRecycle)
   * getIntervalHistogram(histogramToRecycle)} or {@link
   * SingleWriterRecorder#getIntervalHistogram()}.
   *
   * <p>NOTE: The caller is responsible for not recycling the same returned interval histogram more
   * than once. If the same interval histogram instance is recycled more than once, behavior is
   * undefined.
   *
   * <p>Calling {@link SingleWriterRecorder#getIntervalHistogram(Histogram histogramToRecycle)
   * getIntervalHistogram(histogramToRecycle)} will reset the value counts, and start accumulating
   * value counts for the next interval
   *
   * @param histogramToRecycle a previously returned interval histogram that may be recycled to
   *     avoid allocation and copy operations.
   * @return a histogram containing the value counts accumulated since the last interval histogram
   *     was taken.
   */
  public synchronized Histogram getIntervalHistogram(Histogram histogramToRecycle) {
    ...
    inactiveHistogram =
        null; // Once we expose the sample, we can't reuse it internally until it is recycled
    return sampledHistogram;
  }

  /**
   * Place a copy of the value counts accumulated since accumulated (since the last interval
   * histogram was taken) into {@code targetHistogram}.
   *
   * <p>Calling {@link SingleWriterRecorder#getIntervalHistogramInto getIntervalHistogramInto()}
   * will reset the value counts, and start accumulating value counts for the next interval.
   *
    ...
    if (bad) {
      throw new IllegalArgumentException(
          "replacement histogram must have been obtained via a previous"
              + " getIntervalHistogram() call from this "
