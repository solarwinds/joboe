 ...
 *
 *
 * <h3>A floating point values High Dynamic Range (HDR) Histogram that is synchronized as a whole
 * </h3>
 *
 * <p>A {@link SynchronizedDoubleHistogram} is a variant of {@link DoubleHistogram} that is
 * synchronized as a whole, such that queries, copying, and addition operations are atomic with
 * relation to modification on the {@link SynchronizedDoubleHistogram}, nd such that external
 * accessors (e.g. iterations on the histogram data) that synchronize on the {@link
 * SynchronizedDoubleHistogram} instance can safely assume that no modifications to the histogram
 * data occur within their synchronized block.
 *
 * <p>It is important to note that synchronization can result in blocking recoding calls. If
 * non-blocking recoding operations are required, consider using {@link ConcurrentDoubleHistogram},
 * or (recommended) {@link DoubleRecorder} which were intended for concurrent operations.
 *
 * <p>{@link SynchronizedDoubleHistogram} supports the recording and analyzing sampled data value
 * counts across a configurable dynamic range of floating point (double) values, with configurable
 * value precision within the range. Dynamic range is expressed as a ratio between the highest and
 * lowest non-zero values trackable within the histogram at any given time. Value precision is
 * expressed as the number of significant [decimal] digits in the value recording, and provides
 ...
 * {@link SynchronizedDoubleHistogram} is not specified upfront. Only the dynamic range of values
 * that the histogram can cover is (optionally) specified. E.g. When a {@link
 * ConcurrentDoubleHistogram} is created to track a dynamic range of 3600000000000 (enough to track
 * values from a nanosecond to an hour), values could be recorded into into it in any consistent
 * unit of time as long as the ratio between the highest and lowest non-zero values stays within the
 * specified dynamic range, so recording in units of nanoseconds (1.0 thru 3600000000000.0),
 * milliseconds (0.000001 thru 3600000.0) seconds (0.000000001 thru 3600.0), hours (1/3.6E12 thru
 * 1.0) will all work just as well.
 *
 * <p>Auto-resizing: When constructed with no specified dynamic range (or when auto-resize is turned
 * on with {@link SynchronizedDoubleHistogram#setAutoResize}) a {@link SynchronizedDoubleHistogram}
 * will auto-resize its dynamic range to include recorded values as they are encountered. Note that
 * recording calls that cause auto-resizing may take longer to execute, as resizing incurs
 * allocation and copying of internal data structures.
 *
...
public class SynchronizedDoubleHistogram extends DoubleHistogram {
   ...
   *     separation. Must be a non-negative integer between 0 and 5.
   */
  public SynchronizedDoubleHistogram(
  ...
  @Override
  public synchronized void recordValueWithCount(final double value, final long count)
      throws ArrayIndexOutOfBoundsException {
    super.recordValueWithCount(value, count);
  }

  @Override
  public synchronized void recordValueWithExpectedInterval(
  ...
  public synchronized DoubleHistogram copy() {
    final DoubleHistogram targetHistogram = new DoubleHistogram(this);
    integerValuesHistogram.copyInto(targetHistogram.integerValuesHistogram);
    return targetHistogram;
  }

  @Override
  public synchronized DoubleHistogram copyCorrectedForCoordinatedOmission(
  ...
  public synchronized void copyInto(final DoubleHistogram targetHistogram) {
    // Synchronize copyInto(). Avoid deadlocks by synchronizing in order of construction identity
    // count.
  ...
  @Override
  public synchronized void copyIntoCorrectedForCoordinatedOmission(
      final DoubleHistogram targetHistogram, final double expectedIntervalBetweenValueSamples) {
    // Synchronize copyIntoCorrectedForCoordinatedOmission(). Avoid deadlocks by synchronizing in
    // order
    // of construction identity count.
    if (integerValuesHistogram.identity < targetHistogram.integerValuesHistogram.identity) {
      synchronized (this) {
        synchronized (targetHistogram) {
          super.copyIntoCorrectedForCoordinatedOmission(
              targetHistogram, expectedIntervalBetweenValueSamples);
        }
      }
    } else {
      synchronized (targetHistogram) {
  ...
  @Override
  public synchronized void add(final DoubleHistogram fromHistogram)
      throws ArrayIndexOutOfBoundsException {
  ...
  public synchronized void subtract(final DoubleHistogram fromHistogram) {
    // Synchronize subtract(). Avoid deadlocks by synchronizing in order of construction identity
    // count.
    if (integerValuesHistogram.identity < fromHistogram.integerValuesHistogram.identity) {
      synchronized (this) {
        synchronized (fromHistogram) {
  ...
  @Override
  public synchronized void addWhileCorrectingForCoordinatedOmission(
    ...
    if (integerValuesHistogram.identity < fromHistogram.integerValuesHistogram.identity) {
      synchronized (this) {
        synchronized (fromHistogram) {
          super.addWhileCorrectingForCoordinatedOmission(
              fromHistogram, expectedIntervalBetweenValueSamples);
        }
      }
    } else {
      synchronized (fromHistogram) {
  ...
  @Override
  public synchronized long getCountAtValue(final double value)
      throws ArrayIndexOutOfBoundsException {
  ...
  @Override
  public synchronized LogarithmicBucketValues logarithmicBucketValues(
  ...
  @Override
  public synchronized void outputPercentileDistribution(
  ...
  public synchronized void outputPercentileDistribution(
    ...
    super.outputPercentileDistribution(
        printStream, percentileTicksPerHalfDistance, outputValueUnitScalingRatio);
  }
