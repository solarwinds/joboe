...
/**
 * Used for iterating through {@link DoubleHistogram} values values according to percentile levels.
 * The iteration is performed in steps that start at 0% and reduce their distance to 100% according
 * to the <i>percentileTicksPerHalfDistance</i> parameter, ultimately reaching 100% when all
...
public class DoublePercentileIterator implements Iterator<DoubleHistogramIterationValue> {
   ...
   * @param percentileTicksPerHalfDistance The number of iteration steps per half-distance to 100%.
   */
  public DoublePercentileIterator(
      final DoubleHistogram histogram, final int percentileTicksPerHalfDistance) {
    this.histogram = histogram;
    integerPercentileIterator =
        new PercentileIterator(histogram.integerValuesHistogram, percentileTicksPerHalfDistance);
    iterationValue =
        new DoubleHistogramIterationValue(integerPercentileIterator.currentIterationValue);
  }
