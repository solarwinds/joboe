...
public abstract class RpcClientManager {
  ...
  static void init(String collector, String collectorCertValue) {
    ...
    URL defaultCertLocation =
        collectorHost.contains("appoptics.com") ? AO_DEFAULT_COLLECTER_CERT_LOCATION : null;
    if (collectorCertValue != null) {
      ...
      logger.info(
          String.format("Setting RPC Client to use bundled Certificate: %s", defaultCertLocation));
      collectorCertLocation = defaultCertLocation;
    }
  }
