...
public class HdrHistogramAdapter implements Histogram {
  ...
  public void recordValue(long value) throws HistogramException {
    ...
    if (value >= MIN_VALUE && value <= MAX_VALUE) {
      ...
      throw new HistogramOutOfRangeException(
          "Value "
              + value
              + " is rejected by the histogram as it is outside of acceptable range "
              + MIN_VALUE
              + " to "
              + MAX_VALUE);
    }
  ...
  public long getSum() {
    return (long)
        (hdrHistogram.getTotalCount()
            * hdrHistogram.getMean()); // close enough... as this histogram does not keep total
  }
