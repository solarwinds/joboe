 ...
 * <i>percentileTicksPerHalfDistance</i> parameter, ultimately reaching 100% when all recorded
...
public class PercentileIterator extends AbstractHistogramIterator
   ...
   * @param percentileTicksPerHalfDistance The number of equal-sized iteration steps per
   *     half-distance to 100%.
   */
  public PercentileIterator(
    ...
    long percentileReportingTicks =
        percentileTicksPerHalfDistance
            * (long)
                Math.pow(
                    2,
                    (long) (Math.log(100.0 / (100.0 - (percentileLevelToIterateTo))) / Math.log(2))
                        + 1);
  ...
  boolean reachedIterationLevel() {
    if (countAtThisValue == 0) return false;
