...
/**
 * Used for iterating through {@link DoubleHistogram} values values using the finest granularity
 * steps supported by the underlying representation. The iteration steps through all possible unit
 * value levels, regardless of whether or not there were recorded values for that value level, and
...
public class DoubleRecordedValuesIterator implements Iterator<DoubleHistogramIterationValue> {
  ...
  public DoubleRecordedValuesIterator(final DoubleHistogram histogram) {
    ...
    iterationValue =
        new DoubleHistogramIterationValue(integerRecordedValuesIterator.currentIterationValue);
  }
