...
/**
 * {@link HistogramLogProcessor} will process an input log and [can] generate two separate log files
 * from a single histogram log file: a sequential interval log file and a histogram percentile
 * distribution log file.
 *
 * <p>The sequential interval log file logs a single stats summary line for each reporting interval.
 *
 * <p>The histogram percentile distribution log file includes a detailed percentiles and fine
 * grained distribution of the entire log file range processed.
 *
 * <p>HistogramLogProcessor will process an input log file when provided with the {@code -i
 * <filename>} option. When no -i option is provided, standard input will be processed.
 *
 * <p>When provided with an output file name {@code <logfile>} with the -o option (e.g. "-o mylog"),
 * HistogramLogProcessor will produce both output files under the names {@code <logfile>} and {@code
 * <logfile>.hgrm} (e.g. mylog and mylog.hgrm).
 *
 * <p>When not provided with an output file name, HistogramLogProcessor will produce [only] the
 * histogram percentile distribution log output to standard output.
 *
 * <p>By default, HistogramLogProcessor only processes hlog file lines lines with no tag specified
 * [aka "default tagged" lines]. An optional -tag parameter can be used to process lines of a
 * [single] specific tag. The -listtags option can be used to list all the tags found in the input
 * file.
 *
 * <p>HistogramLogProcessor accepts optional -start and -end time range parameters. When provided,
 * the output will only reflect the portion of the input log with timestamps that fall within the
 * provided start and end time range parameters.
 *
 * <p>HistogramLogProcessor also accepts and optional -csv parameter, which will cause the output
 * formatting (of both output file forms) to use a CSV file format.
 */
public class HistogramLogProcessor extends Thread {
  ...
  private EncodableHistogram getIntervalHistogram() {
    ...
    try {
      ...
      System.err.println(
          "Log file parsing error at line number "
              + lineNumber
    ...
    try {
      if (config.outputFileName != null) {
        ...
        try {
          ...
          System.err.println(
              "Failed to open percentiles histogram output file " + hgrmOutputFileName);
        }
        if (config.movingWindow) {
          String movingWindowOutputFileName = config.outputFileName + ".mpw";
          try {
            movingWindowLog =
                new PrintStream(new FileOutputStream(movingWindowOutputFileName), false);
            outputTimeRange(
                movingWindowLog,
                "Moving window log for " + config.movingWindowPercentileToReport + " percentile");
          } catch (FileNotFoundException ex) {
            System.err.println(
                "Failed to open moving window output file " + movingWindowOutputFileName);
          }
        }
      }
      ...
      if (intervalHistogram != null) {
        // Shape the accumulated histogram like the histograms in the log file (but clear their
        // contents):
      ...
      while (intervalHistogram != null) {
        ...
        if (intervalHistogram instanceof DoubleHistogram) {
          if (accumulatedDoubleHistogram == null) {
            throw new IllegalStateException(
                "Encountered a DoubleHistogram line in a log of Histograms.");
          }
          accumulatedDoubleHistogram.add((DoubleHistogram) intervalHistogram);
        } else {
          if (accumulatedRegularHistogram == null) {
            throw new IllegalStateException(
                "Encountered a Histogram line in a log of DoubleHistograms.");
          }
        ...
        if (timeIntervalLog != null) {
          if (!timeIntervalLogLegendWritten) {
            ...
            if (config.logFormatCsv) {
              timeIntervalLog.println(
                  "\"Timestamp\",\"Int_Count\",\"Int_50%\",\"Int_90%\",\"Int_Max\",\"Total_Count\","
          ...
          if (intervalHistogram instanceof DoubleHistogram) {
            timeIntervalLog.format(
                ...
                ((DoubleHistogram) intervalHistogram).getValueAtPercentile(50.0)
                    / config.outputValueUnitRatio,
                ((DoubleHistogram) intervalHistogram).getValueAtPercentile(90.0)
                    / config.outputValueUnitRatio,
                ...
                accumulatedDoubleHistogram.getValueAtPercentile(99.99)
                    / config.outputValueUnitRatio,
            ...
            timeIntervalLog.format(
                ...
                ((Histogram) intervalHistogram).getValueAtPercentile(50.0)
                    / config.outputValueUnitRatio,
                ((Histogram) intervalHistogram).getValueAtPercentile(90.0)
                    / config.outputValueUnitRatio,
                ((Histogram) intervalHistogram).getMaxValue() / config.outputValueUnitRatio,
                // values recorded from the beginning until now
                accumulatedRegularHistogram.getTotalCount(),
                accumulatedRegularHistogram.getValueAtPercentile(50.0)
                    / config.outputValueUnitRatio,
                accumulatedRegularHistogram.getValueAtPercentile(90.0)
                    / config.outputValueUnitRatio,
                accumulatedRegularHistogram.getValueAtPercentile(99.0)
                    / config.outputValueUnitRatio,
                accumulatedRegularHistogram.getValueAtPercentile(99.9)
                    / config.outputValueUnitRatio,
                accumulatedRegularHistogram.getValueAtPercentile(99.99)
                    / config.outputValueUnitRatio,
        ...
        if (movingWindowLog != null) {
          if (!movingWindowLogLegendWritten) {
            ...
            if (config.logFormatCsv) {
              movingWindowLog.println(
                  "\"Timestamp\",\"Window_Count\",\""
                      + config.movingWindowPercentileToReport
                      + "%'ile\",\"Max\"");
            } else {
              movingWindowLog.println(
                  "Time: WindoCount " + config.movingWindowPercentileToReport + "%'ile Max");
            }
          }
          if (intervalHistogram instanceof DoubleHistogram) {
            movingWindowLog.format(
                Locale.US,
                movingWindowLogFormat,
                ((intervalHistogram.getEndTimeStamp() / 1000.0) - logReader.getStartTimeSec()),
                // values recorded during the last reporting interval
                ((DoubleHistogram) movingWindowSumHistogram).getTotalCount(),
                ((DoubleHistogram) movingWindowSumHistogram)
                        .getValueAtPercentile(config.movingWindowPercentileToReport)
                    / config.outputValueUnitRatio,
                ((DoubleHistogram) movingWindowSumHistogram).getMaxValue()
                    / config.outputValueUnitRatio);
          } else {
            movingWindowLog.format(
                Locale.US,
                ...
                ((Histogram) movingWindowSumHistogram)
                        .getValueAtPercentile(config.movingWindowPercentileToReport)
                    / config.outputValueUnitRatio,
      ...
      if (accumulatedDoubleHistogram != null) {
        accumulatedDoubleHistogram.outputPercentileDistribution(
            histogramPercentileLog,
            config.percentilesOutputTicksPerHalf,
            config.outputValueUnitRatio,
            config.logFormatCsv);
      } else {
        if (accumulatedRegularHistogram == null) {
        ...
        accumulatedRegularHistogram.outputPercentileDistribution(
            histogramPercentileLog,
            config.percentilesOutputTicksPerHalf,
            config.outputValueUnitRatio,
            config.logFormatCsv);
  ...
  /**
   * Construct a {@link HistogramLogProcessor} with the given arguments (provided in command line
   * style).
   *
   * <pre>
   * [-h]                        help
