 ...
 *
 * <p>It is important to note that {@link DoubleHistogram} is not thread-safe, and does not support
 * safe concurrent recording by multiple threads. If concurrent operation is required, consider
 * usings {@link ConcurrentDoubleHistogram}, {@link SynchronizedDoubleHistogram}, or(recommended)
 * {@link DoubleRecorder}, which are intended for this purpose.
 *
 * <p>{@link DoubleHistogram} supports the recording and analyzing sampled data value counts across
 * a configurable dynamic range of floating point (double) values, with configurable value precision
 * within the range. Dynamic range is expressed as a ratio between the highest and lowest non-zero
 * values trackable within the histogram at any given time. Value precision is expressed as the
 * number of significant [decimal] digits in the value recording, and provides control over value
 * quantization behavior across the value range and the subsequent value resolution at any given
 * level.
 *
 * <p>Auto-ranging: Unlike integer value based histograms, the specific value range tracked by a
 * {@link DoubleHistogram} is not specified upfront. Only the dynamic range of values that the
 * histogram can cover is (optionally) specified. E.g. When a {@link DoubleHistogram} is created to
 ...
 *
 * <p>Auto-resizing: When constructed with no specified dynamic range (or when auto-resize is turned
 * on with {@link DoubleHistogram#setAutoResize}) a {@link DoubleHistogram} will auto-resize its
 * dynamic range to include recorded values as they are encountered. Note that recording calls that
 * cause auto-resizing may take longer to execute, as resizing incurs allocation and copying of
 * internal data structures.
 *
 * <p>Attempts to record non-zero values that range outside of the specified dynamic range (or
 * exceed the limits of of dynamic range when auto-resizing) may results in {@link
 * ArrayIndexOutOfBoundsException} exceptions, either due to overflow or underflow conditions. These
 * exceptions will only be thrown if recording the value would have resulted in discarding or losing
 * the required value precision of values already recorded in the histogram.
 *
 * <p>See package description for {@link com.solarwinds.metrics.hdrHistogram} for details.
 */
public class DoubleHistogram extends EncodableHistogram implements Serializable {
  static final double
      highestAllowedValueEver; // A value that will keep us from multiplying into infinity.
  ...
  /**
   * -- GETTER -- Get the current conversion ratio from interval integer value representation to
   * double units. (keep in mind that this can change because it is auto ranging). This ratio can be
   * useful for converting integer values found in iteration, although the preferred form for
   * accessing iteration values would be to use the
   *
   * <p>and
   *
   * <p>accessors to iterated values.
   *
   * @return the current conversion ratio from interval integer value representation to double
   *     units.
   */
  @Getter volatile double integerToDoubleValueConversionRatio;

  @Getter private boolean autoResize = false;

  /**
   * Construct a new auto-resizing DoubleHistogram using a precision stated as a number of
   ...
   * significant decimal digits.
   *
   * <p>The {@link DoubleHistogram} will use the specified AbstractHistogram subclass for tracking
   * internal counts (e.g. {@link Histogram}, {@link ConcurrentHistogram}, {@link
   * SynchronizedHistogram}, {@link IntCountsHistogram}, {@link ShortCountsHistogram}).
   *
   * @param numberOfSignificantValueDigits Specifies the precision to use. This is the number of
   *     significant decimal digits to which the histogram will maintain value resolution and
   *     separation. Must be a non-negative integer between 0 and 5.
  ...
  /**
   * Construct a new DoubleHistogram with the specified dynamic range (provided in {@code
   * highestToLowestValueRatio}) and using a precision stated as a number of significant decimal
   * digits.
   *
   ...
   */
  public DoubleHistogram(
      final long highestToLowestValueRatio, final int numberOfSignificantValueDigits) {
  ...
  private DoubleHistogram(
    ...
    try {
      ...
      if ((highestToLowestValueRatio * Math.pow(10.0, numberOfSignificantValueDigits))
          >= (1L << 61)) {
      ...
      long integerValueRange =
          deriveIntegerValueRange(highestToLowestValueRatio, numberOfSignificantValueDigits);
      ...
      if (internalCountsHistogram == null) {
        ...
        valuesHistogram =
            histogramConstructor.newInstance(
                1L, (integerValueRange - 1), numberOfSignificantValueDigits);

        // We want the auto-ranging to tend towards using a value range that will result in using
        // the
        // lower tracked value ranges and leave the higher end empty unless the range is actually
        // used.
        // This is most easily done by making early recordings force-shift the lower value limit to
        // accommodate them (forcing a force-shift for the higher values would achieve the
        // opposite).
        ...
        if ((internalCountsHistogram.getLowestDiscernibleValue() != 1)
            ...
            || internalCountsHistogram.getNumberOfSignificantValueDigits()
                != numberOfSignificantValueDigits) {
          throw new IllegalStateException(
              "integer values histogram does not match stated parameters.");
        }
        valuesHistogram = internalCountsHistogram;
        // Derive initialLowestValueInAutoRange from valuesHistogram's
  ...
  /**
   * Construct a {@link DoubleHistogram} with the same range settings as a given source, duplicating
  ...
  public DoubleHistogram(final DoubleHistogram source) {
    this(
        source.configuredHighestToLowestValueRatio,
    ...
    setTrackableValueRange(
        source.currentLowestValueInAutoRange, source.currentHighestValueLimitInAutoRange);
  }

  private void init(
      final long configuredHighestToLowestValueRatio,
      final double lowestTrackableUnitValue,
    ...
    setTrackableValueRange(
        lowestTrackableUnitValue, lowestTrackableUnitValue * internalHighestToLowestValueRatio);
  }

  private void setTrackableValueRange(
      final double lowestValueInAutoRange, final double highestValueInAutoRange) {
    ...
    this.integerToDoubleValueConversionRatio =
        lowestValueInAutoRange / getLowestTrackingIntegerValue();
    this.doubleToIntegerValueConversionRatio = 1.0 / integerToDoubleValueConversionRatio;
    integerValuesHistogram.setIntegerToDoubleValueConversionRatio(
        integerToDoubleValueConversionRatio);
  }
   ...
   * @throws ArrayIndexOutOfBoundsException (may throw) if value is cannot be covered by the
   *     histogram's range
   */
   ...
   *     histogram's range
   */
  public void recordValueWithCount(final double value, final long count)
   ...
   * Record a value in the histogram.
   *
   * <p>To compensate for the loss of sampled values when a recorded value is larger than the
   * expected interval between value samples, Histogram will auto-generate an additional series of
   * decreasingly-smaller (down to the expectedIntervalBetweenValueSamples) value records.
   *
   * <p>Note: This is a at-recording correction method, as opposed to the post-recording correction
   * method provided by {@link #copyCorrectedForCoordinatedOmission(double)}. The use cases for
   * these two methods are mutually exclusive, and only one of the two should be be used on a given
   * data set to correct for the same coordinated omission issue.
   *
   * <p>See notes in the description of the Histogram calls for an illustration of why this
   * corrective behavior is important.
   *
   * @param value The value to record
   * @param expectedIntervalBetweenValueSamples If expectedIntervalBetweenValueSamples is larger
   *     than 0, add auto-generated value records as appropriate if value is larger than
   *     expectedIntervalBetweenValueSamples
   * @throws ArrayIndexOutOfBoundsException (may throw) if value is cannot be covered by the
   *     histogram's range
   */
  public void recordValueWithExpectedInterval(
      final double value, final double expectedIntervalBetweenValueSamples)
      throws ArrayIndexOutOfBoundsException {
    recordValueWithCountAndExpectedInterval(value, 1, expectedIntervalBetweenValueSamples);
  }

  private void recordCountAtValue(final long count, final double value)
      throws ArrayIndexOutOfBoundsException {
  ...
  private void recordValueWithCountAndExpectedInterval(
      final double value, final long count, final double expectedIntervalBetweenValueSamples)
      throws ArrayIndexOutOfBoundsException {
    recordCountAtValue(count, value);
  ...
  private synchronized void autoAdjustRangeForValueSlowPath(final double value) {
    try {
      if (value < currentLowestValueInAutoRange) {
        ...
        do {
          // If value is an exact whole multiple of currentHighestValueLimitInAutoRange, it
          // "belongs" with
          // the next level up, as it crosses the limit. With floating point values, the simplest
          // way to
          // make this shift on exact multiple values happen (but not for any
          // just-smaller-than-exact-multiple
          // values) is to use a value that is 1 ulp bigger in computing the ratio for the shift
          // amount:
          int shiftAmount =
              findCappedContainingBinaryOrderOfMagnitude(
  ...
  private void shiftCoveredRangeToTheRight(final int numberOfBinaryOrdersOfMagnitude) {
    ...
    try {
      ...
      // First, temporarily change the highest value in auto-range without changing conversion
      // ratios.
      ...
      // not end up with any concurrently recorded values that would need to be discarded if the
      // shift
  ...
  private void shiftCoveredRangeToTheLeft(final int numberOfBinaryOrdersOfMagnitude) {
    ...
    try {
      ...
      // First, temporarily change the lowest value in auto-range without changing conversion
      // ratios.
      ...
      // Shift integer histogram right, decreasing the recorded integer values for current
      // recordings
      // by a factor of (1 << numberOfBinaryOrdersOfMagnitude):
  ...
  private void handleShiftValuesException(final int numberOfBinaryOrdersOfMagnitude, Exception ex) {
    if (!autoResize) {
      throw new ArrayIndexOutOfBoundsException(
          "Value outside of histogram covered range.\nCaused by: " + ex);
    }

    long highestTrackableValue = integerValuesHistogram.getHighestTrackableValue();
    int currentContainingOrderOfMagnitude =
        findContainingBinaryOrderOfMagnitude(highestTrackableValue);
    int newContainingOrderOfMagnitude =
        numberOfBinaryOrdersOfMagnitude + currentContainingOrderOfMagnitude;
    if (newContainingOrderOfMagnitude > 63) {
      throw new ArrayIndexOutOfBoundsException(
          "Cannot resize histogram covered range beyond (1L << 63) / (1L << "
              + (integerValuesHistogram.subBucketHalfCountMagnitude)
              + ") - 1.\n"
              + "Caused by: "
              + ex);
    }
  ...
  public DoubleHistogram copy() {
    final DoubleHistogram targetHistogram =
        new DoubleHistogram(
            configuredHighestToLowestValueRatio, getNumberOfSignificantValueDigits());
    targetHistogram.setTrackableValueRange(
   ...
   * additional series of decreasingly-smaller (down to the expectedIntervalBetweenValueSamples)
   * value records for each count found in the current histogram that is larger than the
   * expectedIntervalBetweenValueSamples.
   *
   * <p>Note: This is a post-correction method, as opposed to the at-recording correction method
   * provided by {@link #recordValueWithExpectedInterval(double, double)
   * recordValueWithExpectedInterval}. The two methods are mutually exclusive, and only one of the
   * two should be be used on a given data set to correct for the same coordinated omission issue.
   * by
   *
   * <p>See notes in the description of the Histogram calls for an illustration of why this
   * corrective behavior is important.
   ...
   */
  public DoubleHistogram copyCorrectedForCoordinatedOmission(
  ...
  /**
   * Copy this histogram, corrected for coordinated omission, into the target histogram, overwriting
   * it's contents. (see {@link #copyCorrectedForCoordinatedOmission} for more detailed explanation
   * about how correction is applied)
   *
   * @param targetHistogram the histogram to copy into
   * @param expectedIntervalBetweenValueSamples If expectedIntervalBetweenValueSamples is larger
   *     than 0, add auto-generated value records as appropriate if value is larger than
   *     expectedIntervalBetweenValueSamples
   */
  public void copyIntoCorrectedForCoordinatedOmission(
      final DoubleHistogram targetHistogram, final double expectedIntervalBetweenValueSamples) {
    targetHistogram.reset();
    targetHistogram.addWhileCorrectingForCoordinatedOmission(
        this, expectedIntervalBetweenValueSamples);
    targetHistogram.setStartTimeStamp(integerValuesHistogram.startTimeStampMsec);
  ...
  /**
   * Add the contents of another histogram to this one, while correcting the incoming data for
   ...
   * expectedIntervalBetweenValueSamples.
   *
   * <p>Note: This is a post-recording correction method, as opposed to the at-recording correction
   * method provided by {@link #recordValueWithExpectedInterval(double, double)
   * recordValueWithExpectedInterval}. The two methods are mutually exclusive, and only one of the
   * two should be be used on a given data set to correct for the same coordinated omission issue.
   * by
   *
   ...
   * corrective behavior is important.
   *
   * @param fromHistogram Other histogram. highestToLowestValueRatio and
   *     numberOfSignificantValueDigits must match.
   * @param expectedIntervalBetweenValueSamples If expectedIntervalBetweenValueSamples is larger
   *     than 0, add auto-generated value records as appropriate if value is larger than
   *     expectedIntervalBetweenValueSamples
  ...
  public void addWhileCorrectingForCoordinatedOmission(
      final DoubleHistogram fromHistogram, final double expectedIntervalBetweenValueSamples) {
  ...
  public void subtract(final DoubleHistogram otherHistogram) {
    ...
    for (int i = 0; i < arrayLength; i++) {
      ...
      if (otherCount > 0) {
        double otherValue =
            otherIntegerHistogram.valueFromIndex(i)
  ...
  /**
   * get the current lowest (non zero) trackable value the automatically determined range (keep in
   * mind that this can change because it is auto ranging)
   *
  ...
  /**
   * get the current highest trackable value in the automatically determined range (keep in mind
   * that this can change because it is auto ranging)
   *
  ...
  /**
   * get the Dynamic range of the histogram: the configured ratio between the highest trackable
   * value and the lowest trackable non zero value at any given time.
   *
   * @return the dynamic range of the histogram, expressed as the ratio between the highest
   *     trackable value and the lowest trackable non zero value at any given time.
   */
   ...
   * within the histogram's resolution. Where "equivalent" means that value samples recorded for any
   * two equivalent values are counted in a common total count.
   *
  ...
  public double sizeOfEquivalentValueRange(final double value) {
    return integerValuesHistogram.sizeOfEquivalentValueRange(
            (long) (value * doubleToIntegerValueConversionRatio))
        * integerToDoubleValueConversionRatio;
  }
  ...
  public double lowestEquivalentValue(final double value) {
    return integerValuesHistogram.lowestEquivalentValue(
            (long) (value * doubleToIntegerValueConversionRatio))
        * integerToDoubleValueConversionRatio;
  }
   ...
   *
   * @param value The given value
   * @return The highest value that is equivalent to the given value within the histogram's
   *     resolution.
   */
   ...
   * value. Where "equivalent" means that value samples recorded for any two equivalent values are
   * counted in a common total count.
   *
   * @param value The given value
   * @return The value lies in the middle (rounded up) of the range of values equivalent the given
   *     value.
   */
  public double medianEquivalentValue(final double value) {
    return integerValuesHistogram.medianEquivalentValue(
            (long) (value * doubleToIntegerValueConversionRatio))
        * integerToDoubleValueConversionRatio;
  }
   ...
   *
   * @param value The given value
   * @return The next value that is not equivalent to the given value within the histogram's
   *     resolution.
   */
  public double nextNonEquivalentValue(final double value) {
    return integerValuesHistogram.nextNonEquivalentValue(
            (long) (value * doubleToIntegerValueConversionRatio))
        * integerToDoubleValueConversionRatio;
  }
   ...
   * means that value samples recorded for any two equivalent values are counted in a common total
   * count.
   *
   ...
   *
   * @param timeStampMsec the value to set the time stamp to, [by convention] in msec since the
   *     epoch.
   */
  @Override
  ...
  /**
   * Get the value at a given percentile. When the percentile is &gt; 0.0, the value returned is the
   * value that the given the given percentage of the overall recorded value entries in the
   * histogram are either smaller than or equivalent to. When the percentile is 0.0, the value
   * returned is the value that all value entries in the histogram are either larger than or
   * equivalent to.
   *
   * <p>Note that two values are "equivalent" in this statement if {@link
   * DoubleHistogram#valuesAreEquivalent} would return true.
   *
  ...
  public double getValueAtPercentile(final double percentile) {
    return integerValuesHistogram.getValueAtPercentile(percentile)
        * integerToDoubleValueConversionRatio;
  }

  /**
   * Get the percentile at a given value. The percentile returned is the percentile of values
   * recorded in the histogram that are smaller than or equivalent to the given value.
   *
   * <p>Note that two values are "equivalent" in this statement if {@link
   * DoubleHistogram#valuesAreEquivalent} would return true.
   *
  ...
  public double getPercentileAtOrBelowValue(final double value) {
    return integerValuesHistogram.getPercentileAtOrBelowValue(
        (long) (value * doubleToIntegerValueConversionRatio));
  }
   ...
   *
   * @param lowValue The lower value bound on the range for which to provide the recorded count.
   *     Will be rounded down with {@link DoubleHistogram#lowestEquivalentValue
   *     lowestEquivalentValue}.
   * @param highValue The higher value bound on the range for which to provide the recorded count.
   *     Will be rounded up with {@link DoubleHistogram#highestEquivalentValue
   *     highestEquivalentValue}.
   * @return the total count of values recorded in the histogram within the value range that is
   *     {@literal >=} lowestEquivalentValue(<i>lowValue</i>) and {@literal <=}
   *     highestEquivalentValue(<i>highValue</i>)
   */
  ...
  /**
   * Get the count of recorded values at a specific value (to within the histogram resolution at the
   * value level).
   *
  ...
  public long getCountAtValue(final double value) throws ArrayIndexOutOfBoundsException {
    return integerValuesHistogram.getCountAtValue(
        (long) (value * doubleToIntegerValueConversionRatio));
  }

  /**
   * Provide a means of iterating through histogram values according to percentile levels. The
   * iteration is performed in steps that start at 0% and reduce their distance to 100% according to
   * the <i>percentileTicksPerHalfDistance</i> parameter, ultimately reaching 100% when all recorded
   ...
   *     through the histogram using a {@link DoublePercentileIterator}
   */
   ...
   * performed in steps of <i>valueUnitsPerBucket</i> in size, terminating when all recorded
   * histogram values are exhausted.
   *
   * @param valueUnitsPerBucket The size (in value units) of the linear buckets to use
   * @return An {@link Iterable}{@literal <}{@link DoubleHistogramIterationValue}{@literal >}
   *     through the histogram using a {@link DoubleLinearIterator}
   */
  ...
  /**
   * Provide a means of iterating through histogram values at logarithmically increasing levels. The
   * iteration is performed in steps that start at <i>valueUnitsInFirstBucket</i> and increase
   * exponentially according to <i>logBase</i>, terminating when all recorded histogram values are
   * exhausted.
   *
   ...
   * @return An {@link Iterable}{@literal <}{@link DoubleHistogramIterationValue}{@literal >}
   *     through the histogram using a {@link DoubleLogarithmicIterator}
   */
  public LogarithmicBucketValues logarithmicBucketValues(
  ...
  /**
   * Provide a means of iterating through all recorded histogram values using the finest granularity
   ...
   *
   * @return An {@link Iterable}{@literal <}{@link DoubleHistogramIterationValue}{@literal >}
   *     through the histogram using a {@link DoubleRecordedValuesIterator}
   */
   ...
   * supported by the underlying representation. The iteration steps through all possible unit value
   * levels, regardless of whether or not there were recorded values for that value level, and
  ...
  /**
   * An {@link Iterable}{@literal <}{@link DoubleHistogramIterationValue}{@literal >} through the
   * histogram using a {@link DoublePercentileIterator}
   */
  public static class Percentiles implements Iterable<DoubleHistogramIterationValue> {
    ...
    /**
     * @return A {@link DoublePercentileIterator}{@literal <}{@link
     *     DoubleHistogramIterationValue}{@literal >}
     */
    @Override
  ...
  /**
   * An {@link Iterable}{@literal <}{@link DoubleHistogramIterationValue}{@literal >} through the
   * histogram using a {@link DoubleLinearIterator}
   */
  public static class LinearBucketValues implements Iterable<DoubleHistogramIterationValue> {
    ...
    /**
     * @return A {@link DoubleLinearIterator}{@literal <}{@link
     *     DoubleHistogramIterationValue}{@literal >}
     */
    @Override
    public Iterator<DoubleHistogramIterationValue> iterator() {
  ...
  /**
   * An {@link Iterable}{@literal <}{@link DoubleHistogramIterationValue}{@literal >} through the
   * histogram using a {@link DoubleLogarithmicIterator}
   */
  public static class LogarithmicBucketValues implements Iterable<DoubleHistogramIterationValue> {
    ...
    private LogarithmicBucketValues(
        final DoubleHistogram histogram,
        final double valueUnitsInFirstBucket,
        final double logBase) {
      this.histogram = histogram;
    ...
    /**
     * @return A {@link DoubleLogarithmicIterator}{@literal <}{@link
     *     DoubleHistogramIterationValue}{@literal >}
     */
    @Override
    public Iterator<DoubleHistogramIterationValue> iterator() {
  ...
  /**
   * An {@link Iterable}{@literal <}{@link DoubleHistogramIterationValue}{@literal >} through the
   * histogram using a {@link DoubleRecordedValuesIterator}
   */
  public static class RecordedValues implements Iterable<DoubleHistogramIterationValue> {
    ...
    /**
     * @return A {@link DoubleRecordedValuesIterator}{@literal <}{@link
     *     HistogramIterationValue}{@literal >}
     */
    @Override
    public Iterator<DoubleHistogramIterationValue> iterator() {
  ...
  /**
   * An {@link Iterable}{@literal <}{@link DoubleHistogramIterationValue}{@literal >} through the
   * histogram using a {@link DoubleAllValuesIterator}
   */
  ...
  /**
   * Produce textual representation of the value distribution of histogram data by percentile. The
   * distribution is output with exponentially increasing resolution, with each exponentially
   * decreasing half-distance containing five (5) percentile reporting tick points.
   *
   * @param printStream Stream into which the distribution will be output
   *     <p>
   * @param outputValueUnitScalingRatio The scaling factor by which to divide histogram recorded
   *     values units in output
   */
  public void outputPercentileDistribution(
      final PrintStream printStream, final Double outputValueUnitScalingRatio) {
   ...
   * Produce textual representation of the value distribution of histogram data by percentile. The
   * distribution is output with exponentially increasing resolution, with each exponentially
   * decreasing half-distance containing <i>dumpTicksPerHalf</i> percentile reporting tick points.
   *
   * @param printStream Stream into which the distribution will be output
   *     <p>
   * @param percentileTicksPerHalfDistance The number of reporting points per exponentially
   *     decreasing half-distance
   *     <p>
   * @param outputValueUnitScalingRatio The scaling factor by which to divide histogram recorded
   *     values units in output
   */
  public void outputPercentileDistribution(
    ...
    outputPercentileDistribution(
        printStream, percentileTicksPerHalfDistance, outputValueUnitScalingRatio, false);
  }
  ...
  public void outputPercentileDistribution(
    ...
    integerValuesHistogram.outputPercentileDistribution(
        printStream,
  ...
  static boolean isDoubleHistogramCookie(int cookie) {
    return isCompressedDoubleHistogramCookie(cookie)
        || isNonCompressedDoubleHistogramCookie(cookie);
  }
  ...
  public synchronized int encodeIntoByteBuffer(final ByteBuffer buffer) {
    ...
    int relevantLength =
        integerValuesHistogram.getLengthForNumberOfBuckets(
    ...
    if (buffer.capacity() < getNeededByteBufferCapacity(relevantLength)) {
      throw new ArrayIndexOutOfBoundsException(
          "buffer does not have capacity for "
              + getNeededByteBufferCapacity(relevantLength)
              + " bytes");
    }
  ...
  public synchronized int encodeIntoCompressedByteBuffer(
      final ByteBuffer targetBuffer, final int compressionLevel) {
    ...
    return integerValuesHistogram.encodeIntoCompressedByteBuffer(targetBuffer, compressionLevel)
        + 16;
  }
  ...
  static DoubleHistogram constructHistogramFromBuffer(
    ...
    if (isNonCompressedDoubleHistogramCookie(cookie)) {
      valuesHistogram =
          AbstractHistogram.decodeFromByteBuffer(
              buffer, histogramClass, minBarForHighestToLowestValueRatio);
   ...
   * @param minBarForHighestToLowestValueRatio Force highestTrackableValue to be set at least this
   *     high
   * @return The newly constructed DoubleHistogram
   */
  public static DoubleHistogram decodeFromByteBuffer(
      final ByteBuffer buffer, final long minBarForHighestToLowestValueRatio) {
    return decodeFromByteBuffer(buffer, Histogram.class, minBarForHighestToLowestValueRatio);
  }

  /**
   * Construct a new DoubleHistogram by decoding it from a ByteBuffer, using a specified
   * AbstractHistogram subclass for tracking internal counts (e.g. {@link Histogram}, {@link
   * ConcurrentHistogram}, {@link SynchronizedHistogram}, {@link IntCountsHistogram}, {@link
  ...
  public static DoubleHistogram decodeFromByteBuffer(
    ...
    try {
      ...
      DoubleHistogram histogram =
          constructHistogramFromBuffer(
              cookie, buffer, internalCountsHistogramClass, minBarForHighestToLowestValueRatio);
      return histogram;
  ...
  public static DoubleHistogram decodeFromCompressedByteBuffer(
    ...
    return decodeFromCompressedByteBuffer(
        buffer, Histogram.class, minBarForHighestToLowestValueRatio);
  }
   ...
   * {@link AtomicHistogram}, {@link SynchronizedHistogram}, {@link IntCountsHistogram}, {@link
   * ShortCountsHistogram}).
   *
   * @param buffer The buffer to decode from
   * @param internalCountsHistogramClass The class to use for internal counts tracking
  ...
  public static DoubleHistogram decodeFromCompressedByteBuffer(
    ...
    if (!isCompressedDoubleHistogramCookie(cookie)) {
      throw new IllegalArgumentException(
          "The buffer does not contain a compressed DoubleHistogram");
    }
    DoubleHistogram histogram =
        constructHistogramFromBuffer(
            cookie, buffer, internalCountsHistogramClass, minBarForHighestToLowestValueRatio);
  ...
  private long deriveInternalHighestToLowestValueRatio(
      final long externalHighestToLowestValueRatio) {
    // Internal dynamic range needs to be 1 order of magnitude larger than the containing order of
    // magnitude.
    // e.g. the dynamic range that covers [0.9, 2.1) is 2.33x, which on it's own would require 4x
    // range to
    // cover the contained order of magnitude. But (if 1.0 was a bucket boundary, for example, the
    // range
    // will actually need to cover [0.5..1.0) [1.0..2.0) [2.0..4.0), mapping to an 8x internal
    // dynamic range.
    long internalHighestToLowestValueRatio =
  ...
  private long deriveIntegerValueRange(
      final long externalHighestToLowestValueRatio, final int numberOfSignificantValueDigits) {
    long internalHighestToLowestValueRatio =
    ...
    long lowestTackingIntegerValue =
        AbstractHistogram.numberOfSubbuckets(numberOfSignificantValueDigits) / 2;
  ...
  private static int findContainingBinaryOrderOfMagnitude(final long longNumber) {
    int pow2ceiling =
        64 - Long.numberOfLeadingZeros(longNumber); // smallest power of 2 containing value
    return pow2ceiling;
  }
