 ...
 * recording capability is the only thread-safe behavior provided by AtomicHistogram, and that it is
 * not otherwise synchronized. Specifically, AtomicHistogram does not support auto-resizing, does
 * not support value shift operations, and provides no implicit synchronization that would prevent
 * the contents of the histogram from changing during iterations, copies, or addition operations on
 * the histogram. Callers wishing to make potentially concurrent, multi-threaded updates that would
 * safely work in the presence of queries, copies, or additions of histogram objects should either
 * take care to externally synchronize and/or order their access, use the {@link
 * SynchronizedHistogram} variant, or (recommended) use the {@link Recorder} class, which is
...
public class AtomicHistogram extends Histogram {
  ...
  @Override
  public AtomicHistogram copyCorrectedForCoordinatedOmission(
   ...
   */
  public static AtomicHistogram decodeFromByteBuffer(
   ...
   */
  public static AtomicHistogram decodeFromCompressedByteBuffer(
      final ByteBuffer buffer, final long minBarForHighestTrackableValue)
      throws DataFormatException {
    return decodeFromCompressedByteBuffer(
        buffer, AtomicHistogram.class, minBarForHighestTrackableValue);
  }
