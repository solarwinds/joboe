...
//        assertEquals(DEFAULT_FLAGS, settings.getFlags());
    ...
    public void testInvalidArgs() throws Exception {
        ...
        Settings settings;
        com.tracelytics.joboe.rpc.Settings sourceSettings;
        
        //test remote settings that give empty map for args
        sourceSettings = new com.tracelytics.joboe.rpc.Settings(Settings.OBOE_SETTINGS_TYPE_LAYER_SAMPLE_RATE, DEFAULT_FLAGS_STRING, System.currentTimeMillis(), 100000, TTL, "", Collections.<String, ByteBuffer>emptyMap());
        ...
        sourceSettings = new com.tracelytics.joboe.rpc.Settings(Settings.OBOE_SETTINGS_TYPE_LAYER_SAMPLE_RATE, DEFAULT_FLAGS_STRING, System.currentTimeMillis(), 100000, TTL, "", args);
    ...
    public void testInvalidSampleRate() throws Exception {
        ...
        sourceSettings = new com.tracelytics.joboe.rpc.Settings(Settings.OBOE_SETTINGS_TYPE_LAYER_SAMPLE_RATE, DEFAULT_FLAGS_STRING, System.currentTimeMillis(), 1111111, TTL, "", ARGS);
        Client client = new MockRpcClient(Collections.singletonList(sourceSettings));
        fetcher = getFetcher(client);
        settings = fetcher.getSettings();
        ...
        sourceSettings = new com.tracelytics.joboe.rpc.Settings(Settings.OBOE_SETTINGS_TYPE_LAYER_SAMPLE_RATE, DEFAULT_FLAGS_STRING, System.currentTimeMillis(), -1, TTL, "", ARGS);
        fetcher = getFetcher(new MockRpcClient(Collections.singletonList(sourceSettings)));
        settings = fetcher.getSettings();
    ...
    private abstract class TestClient implements Client {
        ...
        protected List<Settings> getClonedSettings() {
            ...
            for (Settings settingsEntry : settings) {
                clonedSettings.add(new com.tracelytics.joboe.rpc.Settings((com.tracelytics.joboe.rpc.Settings) settingsEntry, System.currentTimeMillis())); //create clones with the current timestamp
            }
