...
public class PollingSettingsFetcherTest {
  ...
  static {
    ...
    //                args.put(arg.getKey(), arg.getValue().duplicate()); //duplicate share the same
    // content...but we are read only...as far as they dun share the same pointer we are okay
    //            }
    //            //need to create deep clone for each buffer args
    //            MOCK_SETTINGS.add(new
    // com.solarwinds.joboe.core.rpc.Settings(Settings.OBOE_SETTINGS_TYPE_LAYER_SAMPLE_RATE,
    // DEFAULT_FLAGS_STRING, Agent.currentTimeStamp(), layerSampleRate.getValue(), TTL,
    // layerSampleRate.getKey(), args));
    //        }
