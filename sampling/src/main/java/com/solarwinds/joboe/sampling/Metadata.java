...
public class Metadata {
    ...
    public static final int METADATA_BUF_SIZE = 1 + Constants.TASK_ID_LEN + Constants.OP_ID_LEN + 1 + 3;
    public static final int METADATA_HEX_STRING_SIZE = (1 + Constants.TASK_ID_LEN + Constants.OP_ID_LEN + 1) * 2 + 3;
    ...
    public void unpackMetadata(byte[] buf)
        ...
        if (version  != CURRENT_VERSION) {
            throw new SamplingException("Unexpected version. Found " + version + " but expected " + CURRENT_VERSION);
        }

        int expectedLen = 1 + Constants.TASK_ID_LEN + Constants.OP_ID_LEN + 1 + 3; //header + task id + op id + flags + delimiters
